...
class reldn_head(nn.Module):
    ...
    def forward(
        ...
        if cfg.MODEL.USE_SPATIAL_FEAT:
            ...
            device_id = spo_feat.get_device()
            spt_feat = Variable(torch.from_numpy(spt_feat.astype("float32"))).cuda(
                device_id
            )
...
def binary_prd_losses(
    ttl_cls_scores, multi_prd_labels_int32, keep_pair_class, pos_weight=None
):
    ...
    if pos_weight is None:
        ...
        binary_loss_function = nn.BCEWithLogitsLoss(
            pos_weight=pos_weight.cuda(device_id)
        )
    ...
    prd_labels = Variable(
        torch.from_numpy(multi_prd_labels_int32.astype(np.float32))
    ).cuda(device_id)
...
def triplet_prd_loss(
    ttl_cls_scores,
    multi_prd_labels_int32,
    keep_pair_class,
    s_score,
    o_score,
    pos_weight=None,
    ...
    # prd_cls_scores = ttl_cls_scores[np.arange(0, ttl_cls_scores.shape[0]), keep_pair_class]
    keep_pair_class_id = torch.from_numpy(keep_pair_class.astype(np.int64)).cuda(
        device_id
    )
    keep_pair_class_id = keep_pair_class_id.view(-1, 1)
    prd_cls_scores = torch.gather(ttl_cls_scores, dim=1, index=keep_pair_class_id)
...
def rel_compet_loss(
    ttl_cls_scores, multi_prd_labels_int32, keep_pair_class, rel_mat, rel_mat_cou
):
    push_away_pos_loss, push_away_neg_loss, push_close_pos_loss, push_close_neg_loss = (
        0,
        0,
        0,
        0,
    ...
    push_away_pr_s = normd_ttl_cls_scores[
        keep_pair_n_id, keep_pair_class[keep_pair_n_id]
    ]
    ...
    keep_pair_p_id, keep_pair_p_class = np.where(
        (rel_mat[keep_pair_class] > 0.9) & (rel_mat_cou[keep_pair_class] > 0)
    )
    push_close_pr_o = normd_ttl_cls_scores[keep_pair_p_id, keep_pair_p_class]
    push_close_pr_s = normd_ttl_cls_scores[
        keep_pair_p_id, keep_pair_class[keep_pair_p_id]
    ]
    ...
    if len(push_close_pos_id) > 0:
        ...
        push_close_pos_loss = torch.mean(
            smooth_l1(1 - push_close_pr_o[push_close_pos_id])
        )
...
def reldn_losses(prd_cls_scores, prd_labels_int32, fg_only=False):
    ...
    prd_labels = Variable(torch.from_numpy(prd_labels_int32.astype("int64"))).cuda(
        device_id
    )
...
def reldn_contrastive_losses(prd_scores_sbj_pos, prd_scores_obj_pos, rel_ret):
    ...
    sbj_pair_pos_batch, sbj_pair_neg_batch, sbj_target = split_pos_neg_spo_agnostic(
        prd_probs_sbj_pos,
        rel_ret["binary_labels_sbj_pos_int32"],
        rel_ret["inds_unique_sbj_pos"],
        rel_ret["inds_reverse_sbj_pos"],
    )
    sbj_contrastive_loss = F.margin_ranking_loss(
        sbj_pair_pos_batch,
        sbj_pair_neg_batch,
        sbj_target,
        margin=cfg.MODEL.NODE_CONTRASTIVE_MARGIN,
    )
    # obj
    if cfg.MODEL.MULTI_RELATION:
    ...
    obj_pair_pos_batch, obj_pair_neg_batch, obj_target = split_pos_neg_spo_agnostic(
        prd_probs_obj_pos,
        rel_ret["binary_labels_obj_pos_int32"],
        rel_ret["inds_unique_obj_pos"],
        rel_ret["inds_reverse_obj_pos"],
    )
    obj_contrastive_loss = F.margin_ranking_loss(
        obj_pair_pos_batch,
        obj_pair_neg_batch,
        obj_target,
        margin=cfg.MODEL.NODE_CONTRASTIVE_MARGIN,
...
def reldn_so_contrastive_losses(prd_scores_sbj_pos, prd_scores_obj_pos, rel_ret):
    ...
    sbj_pair_pos_batch, sbj_pair_neg_batch, sbj_target = split_pos_neg_so_aware(
        ...
        rel_ret["inds_unique_sbj_pos"],
        rel_ret["inds_reverse_sbj_pos"],
        rel_ret["sbj_labels_sbj_pos_int32"],
        rel_ret["obj_labels_sbj_pos_int32"],
        "s",
    )
    sbj_so_contrastive_loss = F.margin_ranking_loss(
        sbj_pair_pos_batch,
        sbj_pair_neg_batch,
        sbj_target,
        margin=cfg.MODEL.NODE_CONTRASTIVE_SO_AWARE_MARGIN,
    )
    # obj
    if cfg.MODEL.MULTI_RELATION:
        prd_probs_obj_pos = F.sigmoid(prd_scores_obj_pos)
    ...
    obj_pair_pos_batch, obj_pair_neg_batch, obj_target = split_pos_neg_so_aware(
        ...
        rel_ret["inds_unique_obj_pos"],
        rel_ret["inds_reverse_obj_pos"],
        rel_ret["sbj_labels_obj_pos_int32"],
        rel_ret["obj_labels_obj_pos_int32"],
        "o",
    )
    obj_so_contrastive_loss = F.margin_ranking_loss(
        obj_pair_pos_batch,
        obj_pair_neg_batch,
        obj_target,
        margin=cfg.MODEL.NODE_CONTRASTIVE_SO_AWARE_MARGIN,
...
def reldn_p_contrastive_losses(
    prd_scores_sbj_pos,
    prd_scores_obj_pos,
    prd_bias_scores_sbj_pos,
    prd_bias_scores_obj_pos,
    rel_ret,
):
    # sbj
    if cfg.MODEL.MULTI_RELATION:
    ...
    sbj_p_contrastive_loss = F.margin_ranking_loss(
        ...
        sbj_pair_neg_batch,
        sbj_target,
        margin=cfg.MODEL.NODE_CONTRASTIVE_P_AWARE_MARGIN,
    )
    # obj
    if cfg.MODEL.MULTI_RELATION:
        prd_probs_obj_pos = F.sigmoid(prd_scores_obj_pos)
    ...
    obj_p_contrastive_loss = F.margin_ranking_loss(
        obj_pair_pos_batch,
        obj_pair_neg_batch,
        obj_target,
        margin=cfg.MODEL.NODE_CONTRASTIVE_P_AWARE_MARGIN,
...
def split_pos_neg_spo_agnostic(
    prd_probs, binary_labels_pos, inds_unique_pos, inds_reverse_pos
):
    ...
    pair_pos_batch = torch.ones(1).cuda(
        device_id
    )  # a dummy sample in the batch in case there is no real sample
    pair_neg_batch = torch.zeros(1).cuda(
        device_id
    )  # a dummy sample in the batch in case there is no real sample
    for i in range(inds_unique_pos.shape[0]):
        ...
        if (
            pair_pos_inds.size == 0 or pair_neg_inds.size == 0
        ):  # ignore this node if either pos or neg does not exist
            continue
        ...
        pair_pos_batch = torch.cat(
            (pair_pos_batch, min_prd_pos_probs_i_pair_pos.unsqueeze(0))
        )
        pair_neg_batch = torch.cat(
            (pair_neg_batch, max_prd_pos_probs_i_pair_neg.unsqueeze(0))
        )
...
def split_pos_neg_so_aware(
    ...
    for i in range(inds_unique_pos.shape[0]):
        ...
        max_prd_pos_probs_i_pair_neg = torch.max(
            prd_pos_probs_i_pair_neg
        )  # this is fixed for a given i
        if s_or_o == "s":
            # get all unique object labels
            (
                unique_obj_labels,
                inds_unique_obj_labels,
                inds_reverse_obj_labels,
            ) = np.unique(
            ...
            for j in range(inds_unique_obj_labels.shape[0]):
                ...
                if neg_j_inds.size == 0:
                    if cfg.MODEL.USE_SPO_AGNOSTIC_COMPENSATION:
                        pair_pos_batch = torch.cat(
                            (pair_pos_batch, min_prd_pos_probs_i_pos_j.unsqueeze(0))
                        )
                        pair_neg_batch = torch.cat(
                            (pair_neg_batch, max_prd_pos_probs_i_pair_neg.unsqueeze(0))
                        )
                ...
                pair_neg_batch = torch.cat(
                    (pair_neg_batch, max_prd_pos_probs_i_neg_j.unsqueeze(0))
                )
        else:
            # get all unique subject labels
            (
                unique_sbj_labels,
                inds_unique_sbj_labels,
                inds_reverse_sbj_labels,
            ) = np.unique(
...
def split_pos_neg_p_aware(
    ...
    if cfg.MODEL.MULTI_RELATION:
        ...
        prd_pos_probs = 1 - prd_probs[:, 0]  # shape is (#rels,)
    prd_labels_det = (
        prd_probs[:, 1:].argmax(dim=1).data.cpu().numpy() + 1
    )  # prd_probs is a torch.tensor, exlucding background
    # loop over each group
    pair_pos_batch = torch.ones(1).cuda(
        device_id
    )  # a dummy sample in the batch in case there is no real sample
