...
if cfg.VGG16.IMAGENET_PRETRAINED_WEIGHTS != "":
    logger.info(
        "Loading imagenet pretrained weights from %s",
        cfg.VGG16.IMAGENET_PRETRAINED_WEIGHTS,
    )
...
class VGG16_conv_body(nn.Module):
    def __init__(self):
        super().__init__()
        self.num_layers = 16
        self.spatial_scale = 1.0 / 16.0  # final feature scale wrt. original image scale
        self.dim_out = 512
