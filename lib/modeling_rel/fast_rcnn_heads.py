...
def fast_rcnn_losses(
    ...
    if focloss is None:
        ...
        loss_cls = focloss(cls_score, rois_label)

    bbox_targets = Variable(torch.from_numpy(bbox_targets)).cuda(device_id)
    bbox_inside_weights = Variable(torch.from_numpy(bbox_inside_weights)).cuda(
        device_id
    )
    bbox_outside_weights = Variable(torch.from_numpy(bbox_outside_weights)).cuda(
        device_id
    )
...
def fast_rcnn_cls_losses(cls_score, label_int32):
    # print(cls_score.shape)
    # print(label_int32.shape)
    # print(label_int32)
    # assert False
    device_id = cls_score.get_device()
    rois_label = Variable(torch.from_numpy(label_int32.astype("int64"))).cuda(device_id)
...
class roi_Xconv1fc_head(nn.Module):
    ...
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        ...
        for i in range(cfg.FAST_RCNN.NUM_STACKED_CONVS):
            module_list.extend(
                [nn.Conv2d(dim_in, hidden_dim, 3, 1, 1), nn.ReLU(inplace=True)]
            )
            dim_in = hidden_dim
