...
class Generalized_RCNN(nn.Module):
    def __init__(self):
        ...
        print(
            "{} frozen_stages in I3D".format(i3d_cfg.model["backbone"]["frozen_stages"])
        )
        ...
        self.cluster_tree3d = cluster_tree3d(
            (C, 0, 0),
            feat_combine,
            feat_vec_dim,
            feat_region_vec_dim,
        ...
        embed_vecs = obj_edge_vectors(
            self.obj_class, cfg.MODEL.OBJ_EMBED_FILE_NAME, wv_dim=self.obj_embed_dim
        )
        ...
        self.after_obj_det_embed = nn.Sequential(
            nn.LeakyReLU(0.1), nn.Linear(self.obj_embed_dim, self.Box_Head.dim_out)
        )

        self.obj_tracking = track_frames_from_featmap

        self.restore_obj_feat = nn.Sequential(
            nn.LeakyReLU(0.1), nn.Linear(feat_combine, self.Box_Head.dim_out, bias=True)
        )
        ...
        self.spatio_atten_map = nn.Conv2d(
            C, low_feat_vec_dim, 1, stride=1, padding=0, bias=True
        )

        self.atten_q_map = nn.Conv2d(
            C, low_feat_vec_dim, 1, stride=1, padding=0, bias=True
        )
        ...
        self.spatio_inferA = nn.Sequential(
            nn.Conv2d(
                low_feat_vec_dim, low_feat_vec_dim, 1, stride=1, padding=0, bias=True
            ),
        )
        ...
        self.spatio_infer_fc = nn.Sequential(
            nn.Linear(
                low_feat_vec_dim * pool_size * pool_size, infer_dim_out, bias=True
            ),
        )
        ...
        self.sem_s_e = nn.Sequential(
            nn.LeakyReLU(0.1), nn.Linear(self.obj_embed_dim, infer_dim_out, bias=True)
        )
    ...
    def _init_modules(self):
        ...
        if (
            ...
            or cfg.VGG16.VRD_PRD_PRETRAINED_WEIGHTS != ""
            or cfg.RESNETS.VG_PRD_PRETRAINED_WEIGHTS != ""
            or cfg.VGG16.VG_PRD_PRETRAINED_WEIGHTS != ""
            or cfg.RESNETS.OI_REL_PRD_PRETRAINED_WEIGHTS != ""
            or cfg.VGG16.OI_REL_PRD_PRETRAINED_WEIGHTS != ""
        ):
            if cfg.RESNETS.VRD_PRD_PRETRAINED_WEIGHTS != "":
                logger.info(
                    "loading prd pretrained weights from %s",
                    cfg.RESNETS.VRD_PRD_PRETRAINED_WEIGHTS,
                )
            ...
            if (
                cfg.MODEL.REL_CONV_BODY != ""
                and cfg.RESNETS.PRD_2D_PRETRAINED_WEIGHTS != ""
            ):
                logger.info(
                    "loading prd pretrained weights from %s",
                    cfg.RESNETS.PRD_2D_PRETRAINED_WEIGHTS,
                )
        ...
        if (
            ...
            or cfg.VGG16.TO_BE_FINETUNED_WEIGHTS != ""
        ):
        ...
        if cfg.RESNETS.REL_PRETRAINED_WEIGHTS != "":
            logger.info(
                "loading rel pretrained weights from %s",
        ...
        if cfg.RESNETS.REL_RCNN_PRETRAINED_WEIGHTS != "":
            logger.info(
                "loading rel_rcnn pretrained weights from %s",
                cfg.RESNETS.REL_RCNN_PRETRAINED_WEIGHTS,
            )
            checkpoint = torch.load(
                cfg.RESNETS.REL_RCNN_PRETRAINED_WEIGHTS,
                map_location=lambda storage, loc: storage,
    ...
    def forward(
        ...
        all_frames,
        im_info,
        pre_processed_frames_rpn_ret=None,
        pre_processed_temporal_roi=None,
        do_vis=False,
        dataset_name=None,
        file_name=None,
        roidb=None,
        ...
        **rpn_kwargs
        ...
        if cfg.PYTORCH_VERSION_LESS_THAN_040:
            return self._forward(
                ...
                all_frames,
                im_info,
                pre_processed_frames_rpn_ret,
                pre_processed_temporal_roi,
                do_vis,
                dataset_name,
                file_name,
                roidb,
    ...
    def _forward(
        ...
        **rpn_kwargs
        ...
        if dataset_name is not None:
            ...
            dataset_name = (
                cfg.TRAIN.DATASETS[0] if self.training else cfg.TEST.DATASETS[0]
        ...
        if cfg.ENABLE_FRAME_PRE_PROCESSING and pre_processed_frames_rpn_ret is not None:
            ...
            if self.training:
                ...
                if dataset_name.find("vid") >= 0:
                    pre_processed_frames_rpn_ret["rois"][:, 1:] *= float(
                        im_info[0, -1].item()
                    )
                pre_processed_fg_inds = np.where(
                    pre_processed_frames_rpn_ret["labels_int32"][0] >= 0
                )[0]
                rpn_ret["rois"] = np.vstack(
                    (
                        rpn_ret["rois"],
                        pre_processed_frames_rpn_ret["rois"][pre_processed_fg_inds],
                    )
                )
                rpn_ret["labels_int32"] = np.concatenate(
                    (
                        rpn_ret["labels_int32"],
                        pre_processed_frames_rpn_ret["labels_int32"][0][
        ...
        if self.training:
            ...
            rel_ret = self.RelPN(
                det_rois, det_labels, det_scores, im_info, dataset_name, roidb
            )

            ori_det_feat_vec = self.Box_Head(
                blob_conv, rel_ret, rois_name="det_rois", use_relu=False
            )
            ...
            sbj_feat = self.S_Head(
                blob_conv, rel_ret, rois_name="sbj_rois", use_relu=use_relu
            )
            obj_feat = self.O_Head(
                blob_conv, rel_ret, rois_name="obj_rois", use_relu=use_relu
            )
        else:
            bad_gt = False
            if roidb is not None:
                all_boxes = np.concatenate(
                    (roidb["sbj_gt_boxes"], roidb["obj_gt_boxes"])
                )
                if len(all_boxes) <= 0:
                    bad_gt = True
            if roidb is not None and (not bad_gt):
                ...
                pair_gt_boxes = np.concatenate((sbj_boxes, obj_boxes), axis=1).astype(
                    np.int32
                )
                pair_gt_boxes, pair_gt_index = np.unique(
                    pair_gt_boxes, return_index=True, axis=0
                )
                ...
                unique_all_boxes, gt_boxes_id = np.unique(
                    all_boxes, return_inverse=True, axis=0
                )
                ...
                det_rois_repeated_batch_idx = 0 * blob_utils.ones(
                    (all_rois.shape[0], 1)
                )
                ...
                if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_ROIS:
                    ...
                    for rois_blob_name in rois_blob_names:
                        ...
                        fpn_utils.add_multilevel_roi_blobs(
                            rel_ret,
                            rois_blob_name,
                            rel_ret[rois_blob_name],
                            target_lvls,
                            lvl_min,
                            lvl_max,
                        )
                sbj_det_feat = self.Box_Head(
                    blob_conv, rel_ret, rois_name="sbj_rois", use_relu=True
                )
                sbj_cls_scores, _ = self.Box_Outs(sbj_det_feat)
                sbj_cls_scores = sbj_cls_scores.data.cpu().numpy()
                obj_det_feat = self.Box_Head(
                    blob_conv, rel_ret, rois_name="obj_rois", use_relu=True
                )
                ...
                det_cls_score, det_bbox_pred = self.Box_Outs(
                    self.relu(ori_det_feat_vec)
                )
                ...
                if cfg.DISABLE_EMBED:
                    ...
                    det_embed = self.embed_obj_det(det_cls_soft)
                    det_feat_vec = ori_det_feat_vec + self.after_obj_det_embed(
                        det_embed
                    )
                ...
                rel_ret["sbj_labels"] = (
                    sbj_labels.astype(np.int32, copy=False) + 1
                )  # need to start from 1
                rel_ret["obj_labels"] = (
                    obj_labels.astype(np.int32, copy=False) + 1
                )  # need to start from 1
                rel_ret["all_sbj_labels_int32"] = sbj_labels.astype(
                    np.int32, copy=False
                )
                rel_ret["all_obj_labels_int32"] = obj_labels.astype(
                    np.int32, copy=False
                )

                rel_ret["det_scores"] = det_scores.astype(np.float32, copy=False)
                rel_ret["det_labels"] = (
                    det_labels.astype(np.int32, copy=False) + 1
                )  # need to start from 1
                rel_ret["all_det_labels_int32"] = det_labels.astype(
                    np.int32, copy=False
                )
                ...
                iter_num = 0
                if (
                    cfg.ENABLE_FRAME_PRE_PROCESSING
                    and pre_processed_frames_rpn_ret is not None
                    and dataset_name.find("vidvrd") >= 0
                    and (not cfg.USE_PRD_BOXES)
                ):
                    ...
                    if "prd_category" in rpn_ret:
                        ...
                        if cls_score is not None:
                            ...
                            det_labels = (
                                np.argmax(det_cls_scores_np[:, 1:], axis=1).astype(
                                    np.int32, copy=False
                                )
                                + 1
                            )
                    ...
                    while score_thresh >= -1e-05:  # a negative value very close to 0.0
                        det_rois, det_labels, det_scores, _ = self.prepare_det_rois(
                            rpn_ret["rois"],
                            cls_score,
                            bbox_pred,
                            im_info,
                        ...
                        if score_thresh >= 0.02 - 1e-04:
                            logger.info(
                                "Got {} rel_rois when score_thresh={}, changing to {}".format(
                ...
                det_s_feat = self.S_Head(
                    blob_conv, rel_ret, rois_name="det_rois", use_relu=use_relu
                )
                det_o_feat = self.O_Head(
                    blob_conv, rel_ret, rois_name="det_rois", use_relu=use_relu
                )
        ...
        temporal_roi = self.get_tracklet(
            img_list_f,
            img_list_b,
            rel_ret["det_rois"],
        ...
        (
            roi,
            temporal_roi,
            sampled_temporal_imgs,
            f_resized_scale,
        ) = self.downsample_for_3d(
            roi,
            temporal_roi,
            sampled_temporal_imgs,
            float(1.0 / im_info[0, -1].item()),
            f_scale,
        )

        sampled_temporal_imgs = sampled_temporal_imgs.permute(0, 2, 1, 3, 4)
        temporal_blob_conv_prd = self.Prd_RCNN(
            sampled_temporal_imgs
        )  # n, T, C, P, P -> n, C, T, P, P
        temporal_blob_conv_prd = temporal_blob_conv_prd.permute(
            0, 2, 1, 3, 4
        )  # n, C, T, P, P -> n, T, C, P, P
        cur_id = (
            int(
                math.ceil(
                    bf_cur_len
                    * 1.0
                    / (sampled_temporal_imgs.shape[2] * 1.0)
        ...
        if cfg.FPN.REL_FPN_ON:
            ...
            for ilvl in range(len(temporal_blob_conv_prd)):
                if len(temporal_blob_conv_prd[ilvl].shape) != 5:
                    temporal_blob_conv_prd[ilvl] = temporal_blob_conv_prd[ilvl].view(
                        batch_size,
                        -1,
            ...
            if len(temporal_blob_conv_prd.shape) != 5:
                temporal_blob_conv_prd = temporal_blob_conv_prd.view(
                    batch_size,
                    -1,
                    temporal_blob_conv_prd.shape[1],
        ...
        (
            ...
            batch_tree_bboxnum_list,
            batch_tree_bboxnum_sum_list,
            batch_bboxnum_list,
        ...
        obj_feature, region_feature = self.feat_divide(
            pooled_features, batch_bboxnum_list, batch_tree_bboxnum_sum_list, batch_size
        )
        ...
        spatio_infer_feat_p = self.leakyrelu(
            self.spatio_inferA(spatio_feat1) + self.spatio_inferB(spatio_feat2)
        )
        ...
        sem_infer_feat = self.get_sem_feat(
            obj_feature,
            region_feature,
            lca_list,
            X_list,
        ...
        if sbj_labels is not None:
            sbj_labels = Variable(torch.from_numpy(sbj_labels.astype("int64"))).cuda(
                device_id
            )
        if obj_labels is not None:
            obj_labels = Variable(torch.from_numpy(obj_labels.astype("int64"))).cuda(
                device_id
            )
        if cfg.MODEL.USE_FREQ_BIAS or cfg.MODEL.RUN_BASELINE:
        ...
        if cfg.MODEL.USE_SPATIAL_FEAT:
            spt_feat = Variable(
                torch.from_numpy(rel_ret["spt_feat"].astype("float32"))
            ).cuda(device_id)
        ...
        if self.training:
            ...
            if (
                ...
                rpn_kwargs.update(
                    dict(
                        (k, rpn_ret[k])
                        for k in rpn_ret.keys()
                        if (
                            k.startswith("rpn_cls_logits")
                ...
                if cfg.FPN.FPN_ON:
                    for i, lvl in enumerate(
                        ...
                        return_dict["losses"][
                            "loss_rpn_cls_fpn%d" % lvl
                        ] = loss_rpn_cls[i]
                        return_dict["losses"][
                            "loss_rpn_bbox_fpn%d" % lvl
                        ] = loss_rpn_bbox[i]
                else:
            ...
            if cfg.MODEL.ADD_SCORES_ALL:
                if cfg.MODEL.MULTI_RELATION:
                    loss_cls_ttl, _ = reldn_heads.binary_prd_losses(
                        ttl_cls_scores,
                        rel_ret["multi_prd_labels_int32"],
            ...
            if cfg.MODEL.USE_TRIPLET_PENALTY_LOSS_WEIGHT > 0:
                ...
                loss_cls_triplet_prd = reldn_heads.triplet_prd_loss(
                    ...
                    rel_ret["multi_prd_labels_int32"],
                    rel_ret["keep_pair_class_int32"],
                    max_obj_score[X_list[:, 1]],
                    max_obj_score[Y_list[:, 1]],
                )
        ...
        if cfg.TEST.GET_FRAME_ROIS:  # must use one gpu!
            return_dict["pre_processed_frames_rpn_ret"] = [
                {"rois": rpn_ret["rois"], "labels_int32": rpn_ret["labels_int32"]},
                frames_rpn_ret["rois"],
                None,
            ]
    ...
    def prepare_det_rois(
        ...
        assert (
            rois.shape[0] == scores.shape[0] == box_deltas.shape[0]
        ...
        for im_i in range(batch_size):
            ...
            if (
                dataset_name.find("vidvrd") >= 0
                and cfg.ENABLE_FRAME_PRE_PROCESSING
                and pre_boxes is not None
            ):
                im_boxes = np.tile(pre_boxes[inds, 1:5], (1, im_scores.shape[1]))
            else:
                im_scores, im_boxes = self.get_det_boxes(
                    im_boxes,
                    im_scores,
                    im_box_deltas,
                    im_info[im_i][:2] / im_info[im_i][2],
                )
            ...
            batch_inds = im_i * np.ones((im_boxes.shape[0], 1), dtype=np.float32)
    ...
    def box_results_with_nms_and_limit(
        self, scores, boxes, score_thresh=cfg.TEST.SCORE_THRESH
    ):
    ...
    def roi_feature_transform(
        ...
        """
        assert method in {"RoIPoolF", "RoIAlign"}, "Unknown pooling method: {}".format(
            method
        )
        ...
        if isinstance(blobs_in, list):
            ...
            restore_bl = Variable(
                torch.from_numpy(restore_bl.astype("int64", copy=False))
            ).cuda(device_id)
            ...
            if method == "RoIPoolF":
                xform_out = ROIPool((resolution, resolution), spatial_scale)(
                    blobs_in, rois
                )
            elif method == "RoIAlign":
                xform_out = ROIAlign(
    ...
    def get_tracklet(
        ...
        roi,
        batch_size=None,
        frames_rois_wo_cur=None,
        torch_frames_box_feat_wo_cur=None,
        torch_box_feat=None,
        device_id=None,
        post_prdcnn_T=1,
    ):
        ...
        wh_frames_rois_wo_cur[:, 3] = (
            wh_frames_rois_wo_cur[:, 3] - wh_frames_rois_wo_cur[:, 1] + 1
        )
        wh_frames_rois_wo_cur[:, 4] = (
            wh_frames_rois_wo_cur[:, 4] - wh_frames_rois_wo_cur[:, 2] + 1
        )

        wh_frames_rois_wo_cur = np.hstack(
            [wh_frames_rois_wo_cur[:, 0:5], frames_box_feat_wo_cur]
        )
        ...
        for b in range(batch_size):
            ...
            for t in range(0, len(img_list_f) - 1):
                idx = np.where(
                    abs(wh_frames_rois_wo_cur[:, 0] - (b * batch_size + t)) < 1e-6
                )[0]
            ...
            _, tracklet_2 = self.obj_tracking(
                img_list_b, rois_forward, is_reverse=False
            )
            _, tracklet_1 = self.obj_tracking(
                img_list_f, rois_backward, is_reverse=True
            )
    ...
    def feat_divide(
        ...
        if batch_size == 1:
            return (
                pooled_features[
                    batch_tree_bboxnum_sum_list[0] : batch_tree_bboxnum_sum_list[0]
                    + batch_bboxnum_list[0]
                ],
                pooled_features[
                    batch_tree_bboxnum_sum_list[0]
                    + batch_bboxnum_list[0] : batch_tree_bboxnum_sum_list[1]
                ],
            )
        for b in range(batch_size):
            obj_feature.append(
                pooled_features[
                    batch_tree_bboxnum_sum_list[b] : batch_tree_bboxnum_sum_list[b]
                    + batch_bboxnum_list[b]
                ]
            )
            region_feature.append(
                pooled_features[
                    batch_tree_bboxnum_sum_list[b]
                    + batch_bboxnum_list[b] : batch_tree_bboxnum_sum_list[b + 1]
                ]
            )
    ...
    def get_spatio_atten_feat(
        ...
        for b in range(batch_size):
            ...
            rel_rois = self.get_rel_rois(
                roi[X + batch_bboxnum_sum_list[b]], roi[Y + batch_bboxnum_sum_list[b]]
            )
            rel_rois = dict(rois=rel_rois)

            atten_map_q_lca = self.roi_feature_transform(
                atten_map_q,
                rel_rois,
                resolution=self.pool_size,
                spatial_scale=self.rel_spatial_scale,
            )
            atten_map_lca = self.roi_feature_transform(
                atten_map,
                rel_rois,
                resolution=self.pool_size,
                spatial_scale=self.rel_spatial_scale,
    ...
    def frames_sample(self, tot_frames_len, bf_cur_len):
        ...
        if bf_cur_len <= 0:
            if tot_frames_len - bf_cur_len - 1 >= 1:
                bh_id = (
                    np.random.choice(tot_frames_len - bf_cur_len - 1, 1)[0]
                    + bf_cur_len
                    + 1
                )
    ...
    def saved_tracklet_mapping(self, cur_rois, temporal_roi, bf_cur_len):
        ...
        for i in range(len(cur_rois)):
            idx = np.where(
                np.sum(abs(cur_rois[i, :] - temporal_roi[:, bf_cur_len, :]), axis=1)
                < 4e-1
            )[0]
            if len(idx) <= 0:
                flg = True
                ...
                tmp = np.pad(
                    tmp,
                    ((bf_cur_len, temporal_roi.shape[1] - bf_cur_len - 1), (0, 0)),
                    "constant",
                    constant_values=0,
                )
                tracklet.append(tmp)
