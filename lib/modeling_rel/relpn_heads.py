...
from modeling_rel.generate_rel_proposal_labels import \
    GenerateRelProposalLabelsOp
...
class single_scale_relpn_outputs(nn.Module):
    ...
    def __init__(self):
        ...
        ds_name = (
            cfg.TRAIN.DATASETS[0] if len(cfg.TRAIN.DATASETS) else cfg.TEST.DATASETS[0]
        )
    ...
    def forward(
        ...
        if ((not self.training) or cfg.USE_BASELINE.FULL_GRAPH_MSP) and (
            dataset_name.find("ag") >= 0 and len(person_inds) > 0
        ):  ###!!!
            ...
            if det_rois.shape[0] > 1:
                sbj_inds, obj_inds = self.sbj_adapt_remove_self_pairs(
                    det_rois.shape[0], sbj_inds, obj_inds, person_inds
                )
        else:
            ...
            if (
                det_rois.shape[0] > 1
            ):  # no pairs to remove when there is at most one detection
        ...
        if cfg.MODEL.USE_OVLP_FILTER and lim_score >= 0.015:
            ...
            if (
                ovlp_inds.size > 0
            ):  # only valid when this filtering leaves at least one sample
        ...
        if self.training:
            ...
            if cfg.MODEL.USE_FREQ_BIAS or cfg.MODEL.RUN_BASELINE:
                return_dict["all_sbj_labels_int32"] = (
                    sbj_labels.astype(np.int32, copy=False) - 1
                )  # det_labels start from 1
                return_dict["all_obj_labels_int32"] = (
        ...
        # X_list = return_dict['sbj_inds'][:, 1].astype(np.int)
        # Y_list = return_dict['obj_inds'][:, 1].astype(np.int)
        # print(len(return_dict['det_labels']), return_dict['det_labels'])
        # print(max(X_list), X_list)
        # print(max(Y_list), Y_list)
        # sbjl = return_dict['det_labels'][X_list]
        # objl = return_dict['det_labels'][Y_list]
        # sbj_labels = 1+return_dict['all_sbj_labels_int32'].astype(np.int)
        # obj_labels = 1+return_dict['all_obj_labels_int32'].astype(np.int)
        # obji = np.where(obj_labels != objl)[0]
        # sbji = np.where(sbj_labels != sbjl)[0]
        # print(sbj_labels)
        # print(sbjl)
        # print(obj_labels)
        # print(objl)
        ...
        ##sbj_rois = return_dict['sbj_rois']
        ##obj_rois = return_dict['obj_rois']
        ##sbjr = return_dict['det_rois'][X_list]
        ##objr = return_dict['det_rois'][Y_list]
        ##print(sbj_rois)
        ##print(sbjr)
        ##print(obj_rois)
        ##print(objr)
        ##print(np.where(sbj_rois != sbjr))
        ##print(np.where(obj_rois != objr))
        # assert False
