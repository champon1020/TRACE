...
from utils.net import _CorrectMomentum, _get_lr_change_ratio
    ...
    if cur_lr != new_lr:
        ...
        for ind, param_group in enumerate(optimizer.param_groups):
            ...
            if ind <= 1:  # backbone params
                param_group["lr"] = (
                    cfg.SOLVER.BACKBONE_LR_SCALAR * param_group["lr"]
                )  # 0.1 * param_group['lr']
            param_keys += param_group["params"]
        if (
            cfg.SOLVER.TYPE in ["SGD"]
            and cfg.SOLVER.SCALE_MOMENTUM
            and cur_lr > 1e-7
            and ratio > cfg.SOLVER.SCALE_MOMENTUM_THRESHOLD
        ):
    ...
    for k, v in model.state_dict().items():
        ...
        if kk in ckpt:
            ...
            if v.shape == param.shape:
                ...
                if (
                    kk.find("roi_heads.box_predictor") >= 0
                    or kk.find("Box_Outs.cls_score.weight") >= 0
                ):
                    v[c2a_idx].copy_(param[c2a])
                else:
                    print(
                        "[Loaded net not complete] Parameter[{}] Size Mismatch. Param size: {} -x-> {}".format(
                            kk, v.shape, param.shape
                        )
                    )
        # else:
