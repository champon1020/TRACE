...
def test_net_on_dataset(
    ...
    if (
        dataset_name.find("vg") >= 0
        or dataset_name.find("vrd") >= 0
        or dataset_name.find("ag") >= 0
    ):
        task_evaluation_vg_and_vrd.eval_rel_results(
            all_results, output_dir, args.topk, args.do_val
        )
    else:
...
def test_net(args, dataset_name, proposal_file, output_dir, ind_range=None, gpu_id=0):
    ...
    for i, entry in enumerate(roidb):
        ...
        if (
            len(cfg.TRAIN.DATASETS) > 0 and cfg.TRAIN.DATASETS[0].find("vidvrd") >= 0
        ) or (len(cfg.TEST.DATASETS) > 0 and cfg.TEST.DATASETS[0].find("vidvrd") >= 0):
            im_id_st = 0
        elif (
            len(cfg.TRAIN.DATASETS) > 0 and cfg.TRAIN.DATASETS[0].find("ag") >= 0
        ) or (len(cfg.TEST.DATASETS) > 0 and cfg.TEST.DATASETS[0].find("ag") >= 0):
            im_id_st = 1
        else:
            im_id_st = 1

        all_frames_blob, bf_cur_len, f_scale = get_frames_blob(
            cur_image_abs_path,
            im,
            im_id_st=im_id_st,
        ...
        if args.use_gt_boxes:
            ...
            im_results = im_detect_rels(
                ...
                args.do_vis,
                timers,
                frames_rpn_ret=frames_rpn_ret,
                temporal_roi=temporal_roi,
                bf_cur_len=bf_cur_len,
                f_scale=f_scale,
            )
        ...
        if args.do_val:
            im_results.update(
                dict(
                    gt_sbj_boxes=entry["sbj_gt_boxes"],
...
def get_frames_blob(cur_image_abs_path, im0, im_id_st=1, line_order_frame_pipe=None):
    ...
    for ii, j in enumerate(process_frames_id):
        ...
        if os.path.exists(frame_path):
            ...
            if line_order_frame_pipe is not None and line_order_frame_pipe.frame_in(
                frame_path
            ):
