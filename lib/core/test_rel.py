...
def im_get_det_rels(
    ...
    if return_dict["sbj_rois"] is not None:
        ...
        if do_vis:
            if isinstance(return_dict["blob_conv"], list):
                blob_conv = [
                    b.data.cpu().numpy().squeeze() for b in return_dict["blob_conv"]
                ]
            ...
            if isinstance(return_dict["feat_map"], list):
                feat_map = [
                    b.data.cpu().numpy().squeeze() for b in return_dict["feat_map"]
                ]
            ...
            if isinstance(return_dict["temporal_blob_conv_prd"], list):
                temporal_blob_conv_prd = [
                    b.data.cpu().numpy().squeeze()
                    for b in return_dict["temporal_blob_conv_prd"]
                ]
                temporal_blob_conv_prd = [
                    b.mean(axis=1) for b in temporal_blob_conv_prd
                ]
            elif return_dict["temporal_blob_conv_prd"] is not None:
                temporal_blob_conv_prd = (
                    return_dict["temporal_blob_conv_prd"].data.cpu().numpy().squeeze(0)
                )
    ...
    if return_dict["pre_processed_frames_rpn_ret"] is not None:
        return_dict2["pre_processed_frames_rpn_ret"] = return_dict[
            "pre_processed_frames_rpn_ret"
        ]
        return_dict2["pre_processed_temporal_roi"] = return_dict[
...
def _get_blobs(im, rois, target_scale, target_max_size):
    ...
    if cfg.FPN.FPN_ON:
        ...
        blobs["data"] = processed_im.transpose(
            channel_swap
        )  # B, H, W, C --> B, C, H, W
        ...
        blobs["im_info"] = np.hstack((height, width, im_scale))[np.newaxis, :].astype(
            np.float32
        )
