...
def debug_calu_degree(
    batch_size, batch_nonleaf_node_layer_list, new_roi, batch_bboxnum_sum_list
):
    ans = 0
    minans = 999
    non1avgn = 0
    ...
    for b in range(batch_size):
        ...
        for l in a:
            for k, v in l.items():
                ...
                flatten_rate = flatten_rate * n + roi_area(
                    batch_bboxnum_sum_list[b] + k, new_roi
                ) / (S + 1e-5)
                n = n + 1
                avgans = avgans / n
...
class tree_message_passing2D(nn.Module):
    def __init__(
        self, feat_vec_dim, hidden_dim, mode="tree-gru", low_dim_embed_dim=None
    ):
    ...
    def forward_v1(
        ...
        h = (
            torch.FloatTensor(batch_tree_bboxnum_sum_list[batch_size], self.hidden_dim)
            .type_as(feat_vec)
            .to(device=feat_vec.device)
        )
        h.fill_(0.0)
        ...
        (
            ...
            max_layer_num,
        ) = self.get_cross_batch_nonleaf_node_layer_list(
            batch_nonleaf_node_layer_list, batch_size, batch_tree_bboxnum_sum_list
        )

        cross_batch_A_T = self.get_cross_batch_adjmatrix(
            batch_size, batch_tree_bboxnum_sum_list, batch_A_T, h
        )

        h[cross_batch_son_set_list[0]] = self.tree_gru_bottom_up(
            feat_vec[cross_batch_son_set_list[0]], is_leaf=True
        )
        ...
        for layer_id in range(max_layer_num):
            h[cross_batch_fa_set_list[layer_id]] = self.tree_gru_bottom_up(
                ...
                is_leaf=False,
                curr_layer_A_T=cross_batch_A_T[cross_batch_fa_set_list[layer_id]][
                    :, cross_batch_son_set_list[layer_id]
                ],
            )
        ...
        hd.fill_(0.0)
        hd[cross_batch_fa_set_list[max_layer_num - 1]] = h[
            cross_batch_fa_set_list[max_layer_num - 1]
        ]

        cross_batch_A = self.get_cross_batch_adjmatrix(
            batch_size, batch_tree_bboxnum_sum_list, batch_A, hd
        )
        for layer_id in range(max_layer_num - 1, -1, -1):
            hd[cross_batch_son_set_list[layer_id]] = self.tree_gru_top_down(
                feat_vec[cross_batch_son_set_list[layer_id]],
                hd[cross_batch_fa_set_list[layer_id]],
                cross_batch_A[cross_batch_son_set_list[layer_id]][
                    :, cross_batch_fa_set_list[layer_id]
                ],
                msp_mode=self.mode,
            )
    ...
    def get_cross_batch_adjmatrix(
        self, batch_size, batch_tree_bboxnum_sum_list, batch_adj, h
    ):
        cross_batch_adjmatrix = np.zeros(
            (
        ...
        for b in range(batch_size):
            ...
            cross_batch_adjmatrix[
                batch_tree_bboxnum_sum_list[b] : batch_tree_bboxnum_sum_list[b + 1],
                batch_tree_bboxnum_sum_list[b] : batch_tree_bboxnum_sum_list[b + 1],
            ] = A
        ans = torch.from_numpy(cross_batch_adjmatrix).cuda(h.device)
        return ans

    def get_cross_batch_nonleaf_node_layer_list(
        self, batch_nonleaf_node_layer_list, batch_size, batch_tree_bboxnum_sum_list
    ):
        """
        ...
        for b in range(batch_size):
            ...
            for layer in layer_list:
                ...
                for fa, son in layer.items():
                    ...
                    cross_batch_nonleaf_node_layer_list[layer_id][
                        cross_batch_fa
                    ] = cross_batch_son
                    cross_batch_fa_set_list[layer_id].append(
                        batch_tree_bboxnum_sum_list[b] + fa
                    )
    ...
    def atten_message_passing(self, head_vec, son_vec):
        ...
        feat_vec = head_vec + self.reflect_back(
            (atten_score * v_son).sum(0, keepdim=True)
        )
        return feat_vec
    ...
    def atten_message_passing_gcn(self, f_vec, A):
        ...
        for i in range(len(A)):
            ...
            atten_score = (
                torch.matmul(q_c[neigh], q_n[[i]].t()) / self.sqrt_low_dim_embed_dim
            )
            atten_score = F.softmax(atten_score, 0)
            tmp_i_v = self.reflect_back_nodir(
                (atten_score * v[neigh]).sum(0, keepdim=True)
            )
            ans.append(tmp_i_v)
        ans = f_vec + torch.cat(ans, 0)
        return ans

    def tree_gru_bottom_up(
        self, head_input, h_son_vec=None, is_leaf=False, curr_layer_A_T=None
    ):
        """
        ...
        if is_leaf is True:
            ...
            h = torch.mul(z, h_son_sum) + torch.mul(
                (1 - z), self.tanh(self.Wh(head_input) + fa_g)
            )
        return h
    ...
    def tree_gru_top_down(self, son_input, h_fa_vec, curr_layer_A, msp_mode="tree"):
        ...
        if msp_mode.find("tree") >= 0:
            ...
            h = torch.mul(z, h_fa_vec[fa]) + torch.mul(
                (1 - z), self.tanh(self.Whd(son_input) + self.Uhd(r * h_fa_vec[fa]))
            )
        else:
            ...
            h = torch.mul(z, h_fa_sum) + torch.mul(
                (1 - z), self.tanh(self.Whd(son_input) + fa_g)
            )
        return h
...
class cluster_tree2D(nn.Module):
    def __init__(
        ...
        for i in range(group_coeffi):
            self.tree_message_passing_list.append(
                tree_message_passing2D(
                    feat_combine // group_coeffi,
                    hidden_dim // group_coeffi,
                    mode=msp_mode,
                )
            )
    ...
    def get_A_and_link(
        self, batch_size, batch_tree_bboxnum_list, batch_nonleaf_node_layer_list, device
    ):
        """
        return:
        ...
        for b in range(batch_size):
            A_T = np.zeros(
                (batch_tree_bboxnum_list[b], batch_tree_bboxnum_list[b]),
                dtype=np.float32,
            )
            A = np.zeros(
                (batch_tree_bboxnum_list[b], batch_tree_bboxnum_list[b]),
            ...
            link = (-1) * np.ones(
                (batch_tree_bboxnum_list[b], len(layer_list) + 1), dtype=np.int
            )
            link[batch_tree_bboxnum_list[b] - 1, len(layer_list)] = (
                batch_tree_bboxnum_list[b] - 1
            )
            j = len(layer_list) - 1
            for layer in layer_list:
                for fa, son in layer.items():
    ...
    def forward(self, roi, feat_map, obj_feat_vec):
        ...
        bbox_id, batch_bboxnum_list, batch_bboxnum_sum_list = self.get_bbox_id(
            roi, batch_size
        )

        batch_leaf_bbox = roi[:, 1:5].data.cpu().numpy()

        batch_nonleaf_node_layer_list, batch_tree_node_bbox = self.node_cluster(
            batch_leaf_bbox,
            batch_size,
            batch_bboxnum_list,
        ...
        batch_tree_node_bbox = (
            torch.from_numpy(batch_tree_node_bbox)
            .type(torch.FloatTensor)
            .to(device=roi.device)
        )
        ...
        batch_A, batch_A_T, batch_link = self.get_A_and_link(
            batch_size,
            batch_tree_bboxnum_list,
            batch_nonleaf_node_layer_list,
            roi.device,
        ...
        pooled_region_features = self.fc_r(
            pooled_region_features_map.view(len(pooled_region_features_map), -1)
        )
        ...
        for b in range(batch_size):
            pooled_features.append(
                o2c_feat[
                    batch_bboxnum_sum_list[b] : batch_bboxnum_sum_list[b]
                    + batch_bboxnum_list[b]
                ]
            )
            pooled_features.append(
                r2c_feat[
                    s_non_leaf : s_non_leaf
                    + batch_tree_bboxnum_list[b]
                    - batch_bboxnum_list[b]
                ]
            )
        ...
        for i in range(self.group_coeffi):
            input_group_feat = pooled_features[
                :,
                (self.feat_combine // self.group_coeffi)
                * i : (self.feat_combine // self.group_coeffi)
                * (i + 1),
            ]
            group_aggerated_feat = self.tree_message_passing_list[i](
                batch_tree_node_bbox,
                input_group_feat,
                batch_size,
                batch_nonleaf_node_layer_list,
    ...
    def node_cluster(
        ...
        for b in range(batch_size):
            ...
            if cluster_num <= 0:
                continue
            bbox_feat = batch_bbox_feat[
                batch_bboxnum_sum_list[b] : batch_bboxnum_sum_list[b + 1], :
            ].copy()
            feat2 = None
            if add_batch_bbox_feat is not None:
                feat2 = add_batch_bbox_feat[
                    batch_bboxnum_sum_list[b] : batch_bboxnum_sum_list[b + 1], :
                ].copy()
            l = 0
            r = cluster_num - 1
            for layer_id in range(max_tree_height):
                cluster_num = int(
                    math.ceil(float(cluster_num) / self.cluster_num_partition)
                )
                new_node_dict = self.cluster_algorithm(
                    bbox_feat, cluster_num, l, r, dist_type=dist_type, node_feat2=feat2
                )
                nonleaf_node_layer_list.append(new_node_dict)
                bbox_feat, feat2 = self.get_new_bbox_feat(
                    bbox_feat, new_node_dict, feat_type=feat_type, feat2=feat2
                )
                l = r + 1
                r = r + cluster_num
                if cluster_num <= 0:
            ...
            if cluster_num > 1:
                new_node_dict = self.cluster_algorithm(
                    bbox_feat, 1, l, r, dist_type=dist_type, node_feat2=feat2
                )
                nonleaf_node_layer_list.append(new_node_dict)
                bbox_feat, feat2 = self.get_new_bbox_feat(
                    bbox_feat, new_node_dict, feat_type=feat_type, feat2=feat2
            ...
            bbox_feat = np.hstack(
                (
                    b * np.ones(len(bbox_feat), dtype=np.float32).reshape(-1, 1),
                    bbox_feat,
                )
            )
    ...
    def cluster_algorithm(
        self, bbox_feat, cluster_num, l, r, dist_type="euclid", node_feat2=None
    ):
        """
        ...
        id = self.get_cluster(
            ...
            cluster_num,
            partitial=2,
            cluster_score_dist_type="euclid",
        ...
        distance = self.get_distance(
            bbox_feat[cluster_flat], bbox_feat[combine_flat], dist_type=dist_type
        )
        if node_feat2 is not None:
            distance = distance + self.get_distance(
                node_feat2[cluster_flat], node_feat2[combine_flat], dist_type="cosine"
            )
    ...
    def get_cluster(
        ...
        distance = self.get_distance(
            bbox_feat[X], bbox_feat[Y], dist_type=cluster_score_dist_type
        )
        if node_feat2 is not None:
            distance = distance + self.get_distance(
                node_feat2[X], node_feat2[Y], dist_type="cosine"
            )
    ...
    def get_new_bbox_feat(
        self, ori_bbox_feat, new_node_dict, feat_type="coordinate", feat2=None
    ):
        """
        ...
        if isinstance(im, list):
            ...
            for lvl in range(k_min, k_max + 1):
                ...
                if len(rpn_ret[bl_rois]):
                    rois = Variable(
                        torch.from_numpy(rpn_ret[bl_rois].astype(np.float32))
                    ).cuda(device_id)
                    xform_out = ROIAlign((resolution, resolution), sc, sampling_ratio)(
                        bl_in, rois
                    )
