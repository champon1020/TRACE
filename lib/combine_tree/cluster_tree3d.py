...
class cluster_tree3d(cluster_tree2D):
    def __init__(
        ...
        super(cluster_tree3d, self).__init__(
            feat_map_shape,
            feat_combine,
            feat_vec_dim,
            feat_region_vec_dim,
    ...
    def forward(
        ...
        if isinstance(temporal_feat_map, list):
            ...
            for i in range(len(temporal_feat_map)):
                temporal_feat_map_flat.append(
                    temporal_feat_map[i].view(batch_size * T, C, H, W)
                )
        else:
        ...
        if isinstance(temporal_obj_feat_map, list):
            ...
            for i in range(len(temporal_obj_feat_map)):
                temporal_obj_feat_map_flat.append(
                    temporal_obj_feat_map[i].view(batch_size * T, C, H, W)
                )
        else:
            C, H, W = temporal_obj_feat_map.shape[2:5]
            temporal_obj_feat_map_flat = temporal_obj_feat_map.view(
                batch_size * T, C, H, W
            )

        temporal_batch_leaf_bbox = temporal_roi[:, :, 1:5]
        temporal_batch_leaf_bbox = temporal_batch_leaf_bbox.reshape(-1, 4)
        temporal_batch_boxes_id = [
            i * np.ones((batch_bboxnum_list[i // T], 1), dtype=np.int)
            for i in range(batch_size * T)
        ]
        temporal_batch_boxes_id = np.vstack(temporal_batch_boxes_id)
        temporal_batch_boxes_for_align = np.concatenate(
            [temporal_batch_boxes_id, temporal_batch_leaf_bbox], 1
        ).astype(np.float32)
        ...
        temporal_batch_pooled_obj_features_map = self.temporal_ROIalign_obj(
            temporal_obj_feat_map_flat,
        ...
        if cfg.NO_TEMPORAL_FUSION:
            ...
            for b in range(batch_size):
                ...
                temporal_pooled_obj_features_map = (
                    temporal_batch_pooled_obj_features_map[
                        batch_bboxnum_sum_list[b]
                        * T : batch_bboxnum_sum_list[b + 1]
                        * T
                    ]
                )
                temporal_pooled_obj_features_map = (
                    temporal_pooled_obj_features_map.view(
                        T, n, -1, self.pool_size, self.pool_size
                    )
                )
                temporal_pooled_obj_features_map = (
                    temporal_pooled_obj_features_map.permute(1, 0, 2, 3, 4).contiguous()
                )  # n, t, c, P, P

                temporal_pooled_obj_features = temporal_pooled_obj_features_map
                if self.use_pretrain_BoxHead:
                    temporal_pooled_obj_features = self.temporal_obj_fc(
                        temporal_pooled_obj_features
                    )  # n, T, K
                    temporal_pooled_obj_features = temporal_pooled_obj_features.view(
                        n, T, -1
                    )
                else:
                    temporal_pooled_obj_features = self.temporal_obj_fc(
            ...
            if batch_size == 1:
                ...
                temporal_aggregated_obj_feat = torch.cat(
                    temporal_aggregated_obj_feat, 0
                ).to(device=obj_feat_vec.device)
        ...
        for b in range(batch_size):
            ...
            batch_nonleaf_bboxnum.append(
                batch_tree_bboxnum_list[b] - batch_bboxnum_list[b]
            )
        ...
        pooled_region_features_map = self.ROIalign_region(
            feat_map,
            region_roi_blob,
            resolution=self.pool_size,
            spatial_scale=self.spatial_scale,
        ...
        temporal_nonleaf_node_roi = self.temporal_get_new_bbox_feat(
            batch_nonleaf_node_layer_list,
            batch_tree_bboxnum_sum_list,
            batch_bboxnum_list,
            batch_bboxnum_sum_list,
        ...
        temporal_batch_nonleaf_node_id = [
            i * np.ones((batch_nonleaf_bboxnum[i // T], 1), dtype=np.int)
            for i in range(batch_size * T)
        ]
        temporal_batch_nonleaf_node_id = np.vstack(temporal_batch_nonleaf_node_id)
        temporal_batch_nonleaf_node_for_align = np.concatenate(
            [temporal_batch_nonleaf_node_id, temporal_nonleaf_node_roi_flat], 1
        ).astype(np.float32)
        ...
        temporal_batch_pooled_region_features_map = self.temporal_ROIalign_region(
            temporal_obj_feat_map_flat,
        ...
        if cfg.NO_TEMPORAL_FUSION:
            ...
            for b in range(batch_size):
                ...
                temporal_pooled_region_features_map = (
                    temporal_batch_pooled_region_features_map[
                        batch_nonleaf_bboxnum_sum[b]
                        * T : batch_nonleaf_bboxnum_sum[b + 1]
                        * T
                    ]
                )
                temporal_pooled_region_features_map = (
                    temporal_pooled_region_features_map.view(
                        T, n, -1, self.pool_size, self.pool_size
                    )
                )
                temporal_pooled_region_features_map = (
                    temporal_pooled_region_features_map.permute(
                        1, 0, 2, 3, 4
                    ).contiguous()
                )  # n, t, c, P, P
                ...
                if self.use_pretrain_BoxHead:
                    temporal_pooled_region_features = self.temporal_r_fc(
                        temporal_pooled_region_features
                    )  # n, T, K
                    temporal_pooled_region_features = (
                        temporal_pooled_region_features.view(n, T, -1)
                    )
                else:
                    temporal_pooled_region_features = self.temporal_r_fc(
                        temporal_pooled_region_features
                ...
                atten_r_feat = self.temporal_r_attention(
                    pooled_region_features[
                        batch_nonleaf_bboxnum_sum[b] : batch_nonleaf_bboxnum_sum[b + 1]
                    ],
                    temporal_pooled_region_features,
                )  # n, K
            ...
            if batch_size == 1:
                ...
                temporal_aggregated_region_feat = torch.cat(
                    temporal_aggregated_region_feat, 0
                ).to(device=pooled_region_features.device)
            temporal_aggregated_region_feat = (
                pooled_region_features + temporal_aggregated_region_feat
            )
        ...
        for b in range(batch_size):
            pooled_features.append(
                temporal_aggregated_o2c_feat[
                    batch_bboxnum_sum_list[b] : batch_bboxnum_sum_list[b + 1]
                ]
            )
            pooled_features.append(
                temporal_aggregated_r2c_feat[
                    batch_nonleaf_bboxnum_sum[b] : batch_nonleaf_bboxnum_sum[b + 1]
                ]
            )
        pooled_features = torch.cat(pooled_features, 0).to(
            device=temporal_aggregated_o2c_feat.device
        )
        ...
        if self.group_coeffi > 0:
            aggregated_feat = torch.cat(aggregated_feat, 1).to(
    ...
    def temporal_get_new_bbox_feat(
        self,
        batch_nonleaf_node_layer_list,
        batch_tree_bboxnum_sum_list,
        batch_bboxnum_list,
        ...
        new_node_feat = np.zeros(
            (batch_tree_bboxnum_sum_list[batch_size], T, 4), dtype=np.float32
        )
            ...
            for b in range(batch_size):
                ...
                for layer in layer_list:
                    for fa, son in layer.items():
                        ...
                        new_node_feat[
                            fa + batch_tree_bboxnum_sum_list[b]
                        ] = np.transpose(head_feat)
                        temporal_nonleaf_node_roi.append(
                            new_node_feat[fa + batch_tree_bboxnum_sum_list[b]]
                        )
