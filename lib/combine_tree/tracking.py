...
def track_frames_from_featmap(img_list, bboxes, tracker_type="dsst", is_reverse=False):
    ...
    for j in range(len(bboxes)):
        ...
        tracking_bboxes = tracker(
            anchor_frames,
            tuple(bbox),
            is_multi_tracker=False,
...
def track_frames(
        ...
        for frame_idx in range(int(next_each_frame) - 1, int(each_frame) - 1, -1):
            if is_collective:
                anchor_frames.append(
                    cv2.imread(
                        os.path.join(
                            frames_path, "frame" + str(frame_idx).zfill(4) + ".jpg"
                        )
                    )
                )
            else:
                anchor_frames.append(
                    cv2.imread(
                        os.path.join(frames_path, str(frame_idx).zfill(6) + ".png")
                    )
                )
    else:
    ...
    if is_multi_tracker is False:
        for each_class, bboxes in anchor_bboxes.items():
            for each_bbox in bboxes:
                ...
                tracking_bboxes = tracker(
                    anchor_frames,
                    tuple(bbox),
                    is_multi_tracker=is_multi_tracker,
                    tracker_type=tracker_type,
                )
                for i, each_track_bbox in enumerate(tracking_bboxes):
                    if i + 1 < len(tracking_bboxes):
        ...
        for bbox in bboxes:
            multiTracker.add(
                tracker_select(
                    is_multi_tracker=is_multi_tracker, tracker_type=tracker_type
                ),
                init_frame,
                tuple(bbox),
            )
        ...
        for i, each_frame in enumerate(frames):
            ...
            if i == 0:
                tracker.start_track(
                    img,
                    dlib.rectangle(
                        first_bbox[0], first_bbox[1], first_bbox[2], first_bbox[3]
                    ),
                )
