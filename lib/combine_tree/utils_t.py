...
def weight_init_kaiming_uniform(
    m, fan_mode="fan_in", nonlinearity="relu"
):  ### fan_mode='fan_in', nonlinearity='leaky_relu'
    classname = m.__class__.__name__
    if isinstance(m, nn.Linear) or isinstance(m, nn.Conv2d):
        nn.init.kaiming_uniform_(
            m.weight.data, mode=fan_mode, nonlinearity=nonlinearity
        )
        if m.bias is not None:
...
def load_checkpoint(model, path_ckpt, optimizer=None, scheduler=None):
    ...
    print(
        "=> loaded checkpoint '{}' (epoch {}, best_ac {})".format(
            path_ckpt, start_epoch, best_ac * 100
        )
    )
...
def show_epoch_info(phase, log_path, info, lr=None):
    ...
    print_log(
        log_path,
        "Group Activity Accuracy: %.2f%%, Loss: %.5f, Action Loss: %.5f, Activity Loss: %.5f, Using %.1f seconds"
        % (
            info["activities_acc"],
            info["loss"],
            info["action_loss_meter"],
            info["activity_loss_meter"],
            info["time"],
        ),
    )
...
def build_loss_bbox(bbox_pred, roi_data, fg_cnt):
    ...
    bbox_targets = torch.mul(
        bbox_targets, bbox_inside_weights
    )  ### find the most overlap gt_labels
