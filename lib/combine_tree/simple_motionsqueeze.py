...
class simple_motionsqueeze(nn.Module):
    ...
    def sim_flow(self, input_tem_map, eta=0.01, sig=5.0):
        """
        input_tem_map: N, C, T, H, W
        ...
        # N, Tp, H, H, W
        g_x = torch.exp((inflat_p_vec_x - score_p_inds_x) / (sig**2)) / (
            math.sqrt(2 * math.pi) * sig
        )
        # N, Tp, W, H, W
        g_y = torch.exp((inflat_p_vec_y - score_p_inds_y) / (sig**2)) / (
            math.sqrt(2 * math.pi) * sig
        )
