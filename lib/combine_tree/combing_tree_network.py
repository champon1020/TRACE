...
from combine_tree.tracking import track_frames, track_frames_from_featmap
...
class combing_tree_network(nn.Module):
    def __init__(
        ...
        segment_len=30,
        root="./",
        obj_class_file_path="data/objects.json",
        obj_embed_file_path="data/obj_embed",
        obj_embed_dim=300,
    ):
        ...
        self.obj_visual_embed = nn.Linear(
            C * pool_size * pool_size, feat_vec_dim, bias=True
        )
        ...
        embed_vecs = obj_edge_vectors(
            self.obj_class,
            os.path.join(root, obj_embed_file_path),
            wv_dim=self.obj_embed_dim,
        )
        ...
        self.after_obj_det_embed = nn.Sequential(
            nn.ReLU(), nn.Linear(self.obj_embed_dim, feat_vec_dim)
        )
        ...
        self.restore_obj_feat = nn.Sequential(
            nn.ReLU(), nn.Linear(feat_combine, feat_vec_dim, bias=True)
        )

        self.spatio_atten_s = nn.Sequential(
            nn.ReLU(), nn.Linear(feat_vec_dim, low_feat_vec_dim, bias=True)
        )
        ...
        self.spatio_infer = nn.Sequential(
            ...
            nn.Linear(
                low_feat_vec_dim * pool_size * pool_size, low_feat_vec_dim, bias=True
            ),
        )
    ...
    def forward(
        ...
        (
            ...
            visual_vec,
            temporal_feat_map,
        ) = self.object_detection_one_pic(img, temporal_imgs, bf_cur_len)

        obj_det_embed = self.embed_obj_det(softmax_obj_vec)
        obj_feat_vec = (
            self.obj_sem_embed(obj_det_vec)
            + self.obj_visual_embed(visual_vec)
            + self.after_obj_det_embed(obj_det_embed)
        )
        ...
        (
            ...
            roi, feat_map, obj_feat_vec, temporal_roi, temporal_feat_map
        )
        ...
        spatio_feat, X_list, Y_list, lca_list = self.get_spatio_atten_feat(
            obj_feature,
            pooled_region_features_map,
            batch_bboxnum_list,
            batch_bboxnum_sum_list,
    ...
    def object_detection_one_pic(self, img, temporal_imgs, bf_cur_len):
        ...
        return:
            ...
            temporal_feat_map
        """
        feat_map, roi, obj_det_vec, softmax_obj_vec, visual_vec = self.obj_det_model(
            img, is_generating_rois=True
        )

        imgs_wo_cur = torch.cat(
            [temporal_imgs[:, 0:bf_cur_len], temporal_imgs[:, bf_cur_len + 1 :]], 1
        )
        ...
        temporal_feat_map = temporal_feat_map.view(
            feat_map.shape[0],
            -1,
            feat_map.shape[1],
            feat_map.shape[2],
        ...
        temporal_feat_map = torch.cat(
            [
                temporal_feat_map[:, 0:bf_cur_len],
                feat_map.unsqueeze(1),
                temporal_feat_map[:, bf_cur_len:],
            ],
            1,
        )
    ...
    def get_tracklet(self, img_list_f, img_list_b, roi, batch_size):
        ...
        for b in range(batch_size):
            ...
            tracklet_forward = torch.tensor(
                tracklet, dtype=torch.float
            )  # bbox_num, segment_len, 4

            tracklet = self.obj_tracking(
                img_list_b, cur_batch_roi, tracker_type="dsst", is_reverse=True
            )
            tracklet_back = torch.tensor(tracklet, dtype=torch.float)
            tracklet_back = tracklet_back[
                :, range(tracklet_back.shape[1] - 1, -1, -1), :
            ]
    ...
    def get_sem_feat(
        ...
        for b in range(batch_size):
            ...
            # Y = Y_list[b]
            lca = lca_list[b]
            sem_feat_vec = torch.cat(
                [low_dim_s_feat[X], low_dim_r_feat[lca], low_dim_o_feat[Y]], 1
            )
