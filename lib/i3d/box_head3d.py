...
class Tail_Layer_3D(nn.Module):
    def __init__(
        self, net, net_layer_names, roi_xform_func, spatial_scale, num_stages=3
    ):
        super().__init__()
        ...
        if cfg.I3D_DC5:
            ...
            roi_size = cfg.FAST_RCNN.ROI_XFORM_RESOLUTION // sr + (
                1 if cfg.FAST_RCNN.ROI_XFORM_RESOLUTION % sr != 0 else 0
            )
    ...
    def forward(self, x, use_relu=True):
        if len(self.layer_names) > 0:
            ...
            x = self.last_layer_forward(
                x
            )  # n, C, T, P, P; print(y.shape) #torch.Size([25, 2048, 2, 4, 4])
            x = x.permute(
                0, 2, 1, 3, 4
            ).contiguous()  # n, C, T, P, P -> n, T, c, p, p ;print(y.shape) #torch.Size([25, 2, 2048, 4, 4])
        y = self.map2vec(x, use_relu=use_relu)
        return y


class Box_Head_3D(nn.Module):
    def __init__(self, in_channels, roi_xform_func, spatial_scale):
        super().__init__()
