...
from vidvrd_utils.trajectory import object_trajectory_proposal, traj_iou
...
def _merge_trajs(traj_1, traj_2):
    try:
        ...
        print(
            "{}-{} {}-{}".format(traj_1.pstart, traj_1.pend, traj_2.pstart, traj_2.pend)
        )
    ...
    for i in range(overlap_length):
        ...
        traj_1.rois[traj_1.length() - overlap_length + i] = drectangle(
...
def _traj_iou(traj_1, traj_2):
    ...
    t1.rois = deque(
        itertools.islice(t1.rois, t2.pstart - t1.pstart, t1.pend - t1.pstart)
    )
...
def greedy_relational_association(
        ...
        if i == 0:
            ...
            for pred_idx, pred in enumerate(sorted_pred_list):
                ...
                for r in last_modify_rel_list:
                    if pred[1] == r.triplet() and (
                        straj.pstart < r.fend and otraj.pstart < r.fend
                    ):
    ...
    return [
        rel.serialize(obj_name_mapping, pred_name_mapping)
        for rel in video_relation_list
    ]
