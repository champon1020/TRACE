...
import numpy as np
from dlib import correlation_tracker, drectangle
...
class Trajectory:
    ...
    def __init__(
        self, pstart, pend, rois, score, category, classeme, vsig=None, gt_trackid=-1
    ):
        """
        bbox: drectangle
        """
    ...
    def serialize(self):
        ...
        obj["rois"] = [
            (bbox.left(), bbox.top(), bbox.right(), bbox.bottom()) for bbox in self.rois
        ]
...
def traj_iou(trajs1, trajs2):
    ...
    bboxes1 = np.asarray(
        [
            [[roi.left(), roi.top(), roi.right(), roi.bottom()] for roi in traj.rois]
            for traj in trajs1
        ]
    )
