...
from .pytorch_misc import argsort_desc, intersect_2d
...
def eval_rel_results(
    all_results, output_dir, topk=100, do_val=True, do_vis=False, do_special=False
):
    ...
    for im_i, res in enumerate(tqdm(all_results)):
        ...
        if res["prd_scores"] is None:
            ...
            det_scores_top_vis = det_scores_vis[
                det_scores_inds[:, 0], det_scores_inds[:, 1]
            ]
            ...
            if "prd_scores_bias" in res:
                ...
                det_scores_top_bias = det_scores_bias[
                    det_scores_inds[:, 0], det_scores_inds[:, 1]
                ]
            ...
            if "prd_scores_spt" in res:
                ...
                det_scores_top_spt = det_scores_spt[
                    det_scores_inds[:, 0], det_scores_inds[:, 1]
                ]
        ...
        if do_val:
            ...
            pred_to_gt = _compute_pred_matches(
                gt_labels_spo, det_labels_spo_top, gt_boxes_so, det_boxes_so_top
            )
            if eval_per_img:
                for k in recalls:
                    if len(pred_to_gt):
    ...
    if do_val:
        ...
        if eval_ap:
            ...
            for c in range(prd_cls_num):
                ...
                print(
                    "rel AP for class {}: {:.2f} ({:.6f})".format(
                        rel_prd_cats[c], 100 * ap, float(npos[c]) / float(all_npos)
                    )
                )
            rel_mAP /= 1.0 * prd_cls_num
            ...
            for c in range(prd_cls_num):
                ...
                ap_str += "{:.2f}, ".format(100 * ap)
                print(
                    "phr AP for class {}: {:.2f} ({:.6f})".format(
                        rel_prd_cats[c], 100 * ap, float(npos[c]) / float(all_npos)
                    )
                )
            phr_mAP /= 1.0 * prd_cls_num
...
def _compute_pred_matches(
    ...
    for gt_ind, gt_box, keep_inds in zip(
        ...
        if phrdet:
            ...
            inds = (
                bbox_overlaps(gt_box_union[None], box_union=box_union)[0] >= iou_thresh
            )
