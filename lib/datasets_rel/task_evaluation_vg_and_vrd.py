...
def eval_rel_results(all_results, output_dir, topk=100, do_val=True):
    ...
    if cfg.TEST.DATASETS[0].find("ag") >= 0:
        val_map_list_path = os.path.join(
            cfg.ROOT_DIR, "data", "ag", "annotations/test_videos_list.json"
        )
        ...
        val_map_list_path = os.path.join(
            cfg.ROOT_DIR, "data", "vidvrd", "annotations/train_fname_list.json"
        )
        with open(val_map_list_path, "r") as f:
            val_map_list = json.load(f)
            f.close()
    ...
    for phrdet in eval_sets:
        ...
        for prd_k in prd_k_set:
            ...
            recalls = {1: 0, 3: 0, 5: 0, 10: 0, 20: 0, 50: 0, 100: 0}
            tot_recalls = {1: 0, 3: 0, 5: 0, 10: 0, 20: 0, 50: 0, 100: 0}
            if do_val:
                all_gt_cnt = 0
                video_gt_cnt = {}
                video_recalls = {}
            ...
            for im_i, res in enumerate(tqdm(all_results)):
                ...
                if res["prd_scores"] is None:
                    ...
                    cnt += 1.0 * (det_scores_spo.size <= topk)
                    alcnt += 1.0
            ...
            if do_val:
                ...
                for file_real_name, v in video_recalls.items():
                    for k, vals in v.items():
                        video_recalls[file_real_name][k] = float(vals) / (
                            float(video_gt_cnt[file_real_name]) + 1e-12
                        )
