...
from .dataset_catalog_rel import (ANN_FN, ANN_FN2, ANN_FN3, ANN_FN4, ANN_FN5,
                                  DATASETS, IM_DIR, IM_PREFIX)
...
class JsonDatasetRel(object):
    ...
    def get_roidb(
        ...
        for entry in roidb:
            ...
            if entry["file_name"] in self.rel_anns:
                file_name = os.path.join(
                    f_rois_file_name,
                    str(int(entry["file_name"].split(".")[0])) + ".pkl",
                )
                ...
                entry["file_name"] = self.fname_list[
                    int(entry["file_name"].split(".")[0])
                ]  ###!!!
                ...
                if contain_frames_pkl is not None:
                    if is_load_pre_processed is not True:
                        print("pre_processed_bboxes loaded.")
                        is_load_pre_processed = True

                    if (
                        len(cfg.TRAIN.DATASETS) > 0
                        and cfg.TRAIN.DATASETS[0].find("vidvrd") < 0
                    ) or (
                        len(cfg.TEST.DATASETS) > 0
                        and cfg.TEST.DATASETS[0].find("vidvrd") < 0
                    ):
                        entry["pre_processed_frames_rpn_ret"] = contain_frames_pkl[
                            "pre_processed_frames_rpn_ret"
                        ]
                        entry["pre_processed_temporal_roi"] = contain_frames_pkl[
        ...
        if gt:
            ...
            cache_filepath = os.path.join(
                self.cache_path, self.name + "_rel_gt_roidb.pkl"
            )
            ...
            if not cfg.ENABLE_SAMPLE_TRAIN:
                ...
                cache_filepath = os.path.join(
                    self.cache_path,
                    self.name + "_rel_gt_roidb_sampled_real_mod3_11_15_19_23.pkl",
                )
            ...
            if os.path.exists(cache_filepath) and not cfg.DEBUG:
                ...
                logger.debug(
                    "_add_gt_from_cache took {:.3f}s".format(
                ...
                logger.debug(
                    "_add_gt_annotations took {:.3f}s".format(
                        self.debug_timer.toc(average=False)
                    )
                )
                if not cfg.DEBUG:
        ...
        if proposal_file is not None:
            ...
            logger.debug(
                "_add_proposals_from_file took {:.3f}s".format(
                    self.debug_timer.toc(average=False)
                )
            )
    ...
    def _add_gt_annotations(self, entry):
        ...
        gt_overlaps = np.zeros(
            (num_valid_objs, self.num_classes), dtype=entry["gt_overlaps"].dtype
        )
    ...
    def _add_gt_from_cache(self, roidb, cache_filepath):
        ...
        for entry, cached_entry in zip(roidb, cached_roidb):
            ...
            (
                ...
                prd_gt_overlaps,
                pair_to_gt_ind_map,
            ) = values[: len(self.valid_cached_keys)]
            if self.keypoints is not None:
            ...
            # add relationship annotations
            entry["sbj_gt_boxes"] = np.append(
                entry["sbj_gt_boxes"], sbj_gt_boxes, axis=0
            )
            entry["sbj_gt_classes"] = np.append(entry["sbj_gt_classes"], sbj_gt_classes)
            entry["sbj_gt_overlaps"] = scipy.sparse.csr_matrix(sbj_gt_overlaps)
            entry["obj_gt_boxes"] = np.append(
                entry["obj_gt_boxes"], obj_gt_boxes, axis=0
            )
            entry["obj_gt_classes"] = np.append(entry["obj_gt_classes"], obj_gt_classes)
    ...
    def _init_keypoints(self):
        ...
        if "keypoints" in cat_info[0]:
            ...
            self.keypoints_to_id_map = dict(zip(keypoints, range(len(keypoints))))
...
def add_rel_proposals(
    roidb, sbj_rois, obj_rois, det_rois, scales, sbj_inds, obj_inds, det_labels
):
    ...
    for i, entry in enumerate(roidb):
        ...
        # sbj_gt w/ obj_det
        sbj_gt_boxes_paired_w_det = np.repeat(
            unique_sbj_gt_boxes, im_det_boxes.shape[0], axis=0
        )
        obj_det_boxes_paired_w_gt = np.tile(
            im_det_boxes, (unique_sbj_gt_boxes.shape[0], 1)
        )
        # sbj_det w/ obj_gt
        sbj_det_boxes_paired_w_gt = np.repeat(
            im_det_boxes, unique_obj_gt_boxes.shape[0], axis=0
        )
        obj_gt_boxes_paired_w_det = np.tile(
            unique_obj_gt_boxes, (im_det_boxes.shape[0], 1)
        )
        # sbj_gt w/ obj_gt
        sbj_gt_boxes_paired_w_gt = np.repeat(
            unique_sbj_gt_boxes, unique_obj_gt_boxes.shape[0], axis=0
        )
        obj_gt_boxes_paired_w_gt = np.tile(
            unique_obj_gt_boxes, (unique_sbj_gt_boxes.shape[0], 1)
        )
        ...
        unique_gt_boxes, unique_gt_boxes_id, gt_boxes_id = np.unique(
            gt_boxes_per_batch, return_index=True, return_inverse=True, axis=0
        )
        ...
        gt_labels_per_batch = np.concatenate(
            (entry["sbj_gt_classes"], entry["obj_gt_classes"])
        )
        gt_labels = gt_labels_per_batch[unique_gt_boxes_id]

        det_labels = np.concatenate((det_labels[det_idx], gt_labels + 1))

        _merge_paired_boxes_into_roidb(
            roidb, sbj_box_list, obj_box_list, sbj_id, obj_id, all_boxes, det_labels
        )
        _add_prd_class_assignments(roidb)


def _merge_paired_boxes_into_roidb(
    roidb, sbj_box_list, obj_box_list, sbj_id, obj_id, all_boxes, det_labels
):
    ...
    for i, entry in enumerate(roidb):
        ...
        if len(pair_gt_inds) > 0:
            ...
            pair_to_gt_overlaps = np.minimum(
                sbj_to_gt_overlaps, obj_to_gt_overlaps
            )  #!!!!#####!!!!
            ...
            sbj_I = np.where(sbj_maxes >= 0)[
                0
            ]  # Those boxes with non-zero overlap with gt boxes, get all items

            obj_argmaxes = obj_to_gt_overlaps.argmax(axis=1)
            obj_maxes = obj_to_gt_overlaps.max(axis=1)  # Amount of that overlap
            obj_I = np.where(obj_maxes >= 0)[
                0
            ]  # Those boxes with non-zero overlap with gt boxes, get all items

            pair_argmaxes = pair_to_gt_overlaps.argmax(axis=1)
            pair_maxes = pair_to_gt_overlaps.max(axis=1)  # Amount of that overlap
            pair_I = np.where(pair_maxes >= 0)[
                0
            ]  # Those boxes with non-zero overlap with gt boxes, get all items

            # Record max overlaps with the class of the appropriate gt box
            sbj_gt_overlaps[sbj_I, sbj_gt_classes[sbj_argmaxes[sbj_I]]] = sbj_maxes[
                sbj_I
            ]
            obj_gt_overlaps[obj_I, obj_gt_classes[obj_argmaxes[obj_I]]] = obj_maxes[
                obj_I
            ]

            if cfg.MODEL.MULTI_RELATION:
                gt_proposal_concat = np.concatenate(
                    [sbj_gt_boxes, obj_gt_boxes], 1
                ).astype(np.int)
                _, gt_pair_indices, gt_pair_inverse = np.unique(
                    gt_proposal_concat, return_index=True, return_inverse=True, axis=0
                )

                # condensed_pair_label = [[] for j in range(len(gt_pair_indices))]
                condensed_pair_label = np.zeros(
                    (len(gt_pair_indices), prd_gt_overlaps.shape[1])
                )
                ...
                prd_gt_overlaps[pair_I] = np.multiply(
                    condensed_pair_label[pair2unique_id],
                    pair_maxes[pair_I].reshape(-1, 1),
                )
            else:
                prd_gt_overlaps[
                    pair_I, prd_gt_classes[pair_argmaxes[pair_I]]
                ] = pair_maxes[pair_I]
        ...
        entry["prd_gt_classes"] = -np.ones(
            (num_pairs), dtype=entry["prd_gt_classes"].dtype
        )
...
def _merge_proposal_boxes_into_roidb(roidb, box_list):
    ...
    for i, entry in enumerate(roidb):
        ...
        gt_overlaps = np.zeros(
            (num_boxes, entry["gt_overlaps"].shape[1]), dtype=entry["gt_overlaps"].dtype
        )
        ...
        entry["boxes"] = np.append(
            entry["boxes"], boxes.astype(entry["boxes"].dtype, copy=False), axis=0
        )
        entry["gt_classes"] = np.append(
            entry["gt_classes"], np.zeros((num_boxes), dtype=entry["gt_classes"].dtype)
        )
        ...
        entry["is_crowd"] = np.append(
            entry["is_crowd"], np.zeros((num_boxes), dtype=entry["is_crowd"].dtype)
        )
        entry["box_to_gt_ind_map"] = np.append(
            entry["box_to_gt_ind_map"],
            box_to_gt_ind_map.astype(entry["box_to_gt_ind_map"].dtype, copy=False),
        )
