...
def prepare_mAP_dets(topk_dets, cls_num):
    ...
    for dets in tqdm(topk_dets):
        ...
        for c in range(cls_num):
            ...
            if len(cls_inds):
                ...
                cls_dets[c]["confidence"].append(cls_det_scores)
                cls_dets[c]["BB_s"].append(cls_sbj_boxes)
                cls_dets[c]["BB_o"].append(cls_obj_boxes)
                cls_dets[c]["BB_r"].append(cls_rel_boxes)
                cls_dets[c]["LBL_s"].append(cls_sbj_labels)
                cls_dets[c]["LBL_o"].append(cls_obj_labels)
    ...
    for c in range(cls_num):
        for k, v in cls_dets[c].items():
            if k.find("BB") >= 0:
                cls_dets[c][k] = np.concatenate(cls_dets[c][k], 0)
            else:
                cls_dets[c][k] = np.concatenate(cls_dets[c][k])
...
def ap_eval(image_ids, dets, gts, npos, rel_or_phr=True, ovthresh=0.5):
    """
