...
def extend_with_flipped_entries(roidb, dataset):
    ...
    for entry in roidb:
        ...
        if entry["pre_processed_frames_rpn_ret"] is not None:
            pre_processed_frames_rpn_ret = entry["pre_processed_frames_rpn_ret"].copy()
            oldx1 = pre_processed_frames_rpn_ret["rois"][:, 1].copy()
            oldx2 = pre_processed_frames_rpn_ret["rois"][:, 3].copy()
            pre_processed_frames_rpn_ret["rois"][:, 1] = width - oldx2 - 1
            pre_processed_frames_rpn_ret["rois"][:, 3] = width - oldx1 - 1
            assert (
                pre_processed_frames_rpn_ret["rois"][:, 3]
                >= pre_processed_frames_rpn_ret["rois"][:, 1]
            ).all(), "{}".format(pre_processed_frames_rpn_ret["rois"])
        else:
        ...
        flipped_entry["pre_processed_frames_rpn_ret"] = pre_processed_frames_rpn_ret
...
def filter_for_training(roidb):
    ...
    def is_valid(entry):
        ...
        bg_inds = np.where(
            (overlaps < cfg.TRAIN.BG_THRESH_HI) & (overlaps >= cfg.TRAIN.BG_THRESH_LO)
        )[0]
    ...
    logger.info(
        "Filtered {} roidb entries: {} -> {}".format(num - num_after, num, num_after)
    )
...
def rank_for_training(roidb):
    ...
    if cfg.TRAIN.ASPECT_CROPPING:
        logging.info(
            "Number of entries that need to be cropped: %d. Ratio bound: [%.2f, %.2f]",
            need_crop_cnt,
            RATIO_LO,
            RATIO_HI,
        )
