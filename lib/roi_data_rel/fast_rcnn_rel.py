...
def _sample_pairs(roidb, im_scale, batch_idx):
    ...
    pairs_per_image = int(
        cfg.TRAIN.FG_REL_SIZE_PER_IM / cfg.TRAIN.FG_REL_FRACTION
    )  # need much more pairs since it's quadratic
    ...
    if cfg.MODEL.MULTI_RELATION:
        ...
        fg_pair_inds, fg_pair_class = np.where(
            (prd_gt_overlaps >= cfg.TRAIN.FG_THRESH) & (prd_gt_overlaps <= 1.0 - 1e-4)
        )

        hash_gt_pair_inds = prd_class_num * gt_pair_inds + gt_pair_class
        hash_fg_pair_inds = prd_class_num * fg_pair_inds + fg_pair_class

        fg_pairs_per_this_image = np.minimum(
            fg_pairs_per_image, hash_gt_pair_inds.size + hash_fg_pair_inds.size
        )
        if (
            hash_fg_pair_inds.size > 0
            and fg_pairs_per_this_image > hash_gt_pair_inds.size
        ):
            hash_fg_pair_inds = npr.choice(
                hash_fg_pair_inds,
                size=(fg_pairs_per_this_image - hash_gt_pair_inds.size),
                replace=False,
            )
        ...
        blob_dict = {}
        if cfg.MODEL.USE_BG:
            bg_pair_inds, bg_pair_class_inds = np.where(
                (prd_gt_overlaps < cfg.TRAIN.BG_THRESH_HI)
            )
            ...
            bg_pairs_per_this_image = np.minimum(
                bg_pairs_per_this_image, hash_bg_pair_inds.size
            )
            ...
            keep_pair_inds = np.append(
                hash_fg_pair_inds // prd_class_num, hash_bg_pair_inds // prd_class_num
            )
            keep_pair_class = np.append(
                hash_fg_pair_inds % prd_class_num, hash_bg_pair_inds % prd_class_num
            )
        else:
            multi_prd_labels = np.ones(
                fg_multi_prd_labels.size, dtype=np.int32
            )  # fg_multi_prd_labels
            keep_pair_inds = np.append(hash_fg_pair_inds // prd_class_num)
            keep_pair_class = np.append(hash_fg_pair_inds % prd_class_num)

        blob_dict["multi_prd_labels_int32"] = multi_prd_labels.astype(
            np.int32, copy=False
        )
        blob_dict["keep_pair_class_int32"] = keep_pair_class.astype(
            np.int32, copy=False
        )
        ...
        fg_pair_inds = np.where(
            (max_pair_overlaps >= cfg.TRAIN.FG_THRESH)
            & (max_pair_overlaps <= 1.0 - 1e-4)
        )[0]

        fg_pairs_per_this_image = np.minimum(
            fg_pairs_per_image, gt_pair_inds.size + fg_pair_inds.size
        )
        # Sample foreground regions without replacement
        if fg_pair_inds.size > 0 and fg_pairs_per_this_image > gt_pair_inds.size:
            fg_pair_inds = npr.choice(
                fg_pair_inds,
                size=(fg_pairs_per_this_image - gt_pair_inds.size),
                replace=False,
            )
        ...
        blob_dict = dict(fg_prd_labels_int32=fg_prd_labels.astype(np.int32, copy=False))
        if cfg.MODEL.USE_BG:
            ...
            bg_pairs_per_this_image = pairs_per_image - fg_pairs_per_this_image
            bg_pairs_per_this_image = np.minimum(
                bg_pairs_per_this_image, bg_pair_inds.size
            )
            # Sample foreground regions without replacement
            ...
            all_prd_labels[: fg_pair_inds.size] = (
                fg_prd_labels + 1
            )  # class should start from 1
    ...
    all_boxes_repeated_batch_idx = batch_idx * blob_utils.ones(
        (sampled_all_boxes.shape[0], 1)
    )
    ...
    int_repeated_batch_idx = batch_idx * np.ones(
        (keep_pair_inds.shape[0], 1), dtype=np.int
    )
    blob_dict["sbj_inds"] = np.hstack(
        (repeated_batch_idx, sampled_sbj_inds.reshape(-1, 1))
    )
    blob_dict["obj_inds"] = np.hstack(
        (repeated_batch_idx, sampled_obj_inds.reshape(-1, 1))
    )
    ...
    if (
        cfg.MODEL.USE_NODE_CONTRASTIVE_LOSS
        or cfg.MODEL.USE_NODE_CONTRASTIVE_SO_AWARE_LOSS
        or cfg.MODEL.USE_NODE_CONTRASTIVE_P_AWARE_LOSS
    ):
        ...
        sbj_pos_obj_pos_pair_neg_inds = np.where(
            ...
            & (max_pair_overlaps < cfg.TRAIN.BG_THRESH_HI)
        )[0]
        ...
        if (
            nodes_per_image - sbj_pos_obj_pos_pair_neg_inds.size > 0
            and sbj_pos_obj_neg_pair_neg_inds.size > 0
        ):
            sbj_pos_obj_neg_pair_neg_inds = npr.choice(
                sbj_pos_obj_neg_pair_neg_inds,
                size=int(
                    min(
                        nodes_per_image - sbj_pos_obj_pos_pair_neg_inds.size,
                        sbj_pos_obj_neg_pair_neg_inds.size,
                    )
                ),
                replace=False,
            )
            sbj_pos_pair_neg_inds = np.append(
                sbj_pos_pair_neg_inds, sbj_pos_obj_neg_pair_neg_inds
            )
        ...
        blob_dict["binary_labels_sbj_pos_int32"] = binary_labels_sbj_pos.astype(
            np.int32, copy=False
        )
        ...
        obj_neg_inds_sbj_pos = np.where(max_obj_overlaps_sbj_pos < cfg.TRAIN.FG_THRESH)[
            0
        ]
        ...
        if cfg.MODEL.USE_SPATIAL_FEAT:
            sampled_spt_feat_sbj_pos = box_utils_rel.get_spt_features(
                sampled_sbj_boxes_sbj_pos,
                sampled_obj_boxes_sbj_pos,
                roidb["width"],
                roidb["height"],
            )
        ...
        if (
            nodes_per_image - obj_pos_sbj_pos_pair_neg_inds.size > 0
            and obj_pos_sbj_neg_pair_neg_inds.size
        ):
            obj_pos_sbj_neg_pair_neg_inds = npr.choice(
                obj_pos_sbj_neg_pair_neg_inds,
                size=int(
                    min(
                        nodes_per_image - obj_pos_sbj_pos_pair_neg_inds.size,
                        obj_pos_sbj_neg_pair_neg_inds.size,
                    )
                ),
                replace=False,
            )
            obj_pos_pair_neg_inds = np.append(
                obj_pos_pair_neg_inds, obj_pos_sbj_neg_pair_neg_inds
            )
        ...
        blob_dict["binary_labels_obj_pos_int32"] = binary_labels_obj_pos.astype(
            np.int32, copy=False
        )
        ...
        sbj_neg_inds_obj_pos = np.where(max_sbj_overlaps_obj_pos < cfg.TRAIN.FG_THRESH)[
            0
        ]
        ...
        if cfg.MODEL.USE_SPATIAL_FEAT:
            sampled_spt_feat_obj_pos = box_utils_rel.get_spt_features(
                sampled_sbj_boxes_obj_pos,
                sampled_obj_boxes_obj_pos,
                roidb["width"],
                roidb["height"],
            )
