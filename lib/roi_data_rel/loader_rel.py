...
class RoiDataLoader(data.Dataset):
    ...
    def __getitem__(self, index_tuple):
        ...
        if self._roidb[index]["need_crop"]:
            ...
            sbj_invalid = (sbj_gt_boxes[:, 0] == sbj_gt_boxes[:, 2]) | (
                sbj_gt_boxes[:, 1] == sbj_gt_boxes[:, 3]
            )
            obj_invalid = (obj_gt_boxes[:, 0] == obj_gt_boxes[:, 2]) | (
                obj_gt_boxes[:, 1] == obj_gt_boxes[:, 3]
            )
            ...
            if min_y == 0:
                ...
                if (box_region - size_crop) < 0:
                    ...
                    y_s = (
                        y_s_min
                        if y_s_min == y_s_max
                        else npr.choice(range(y_s_min, y_s_max + 1))
                    )
                else:
                    ...
                    y_s = (
                        min_y
                        if y_s_add == 0
                        else npr.choice(range(min_y, min_y + y_s_add + 1))
                    )
            # Crop the image
            blobs["data"] = blobs["data"][
                :,
                y_s : (y_s + size_crop),
                :,
            ]
            blobs["all_frames"] = blobs["all_frames"][
                :,
                :,
                y_s : (y_s + size_crop),
                :,
            ...
            if min_x == 0:
                ...
                if (box_region - size_crop) < 0:
                    ...
                    x_s = (
                        x_s_min
                        if x_s_min == x_s_max
                        else npr.choice(range(x_s_min, x_s_max + 1))
                    )
                else:
                    ...
                    x_s = (
                        min_x
                        if x_s_add == 0
                        else npr.choice(range(min_x, min_x + x_s_add + 1))
                    )
            # Crop the image
...
def cal_minibatch_ratio(ratio_list):
    ...
    num_minibatch = int(
        np.ceil(DATA_SIZE / cfg.TRAIN.IMS_PER_BATCH)
    )  # Include leftovers
...
def pad_image_data(list_of_blobs):
    ...
    f_max_shape = blob_utils.get_max_shape(
        [blobs["all_frames"].shape[2:] for blobs in list_of_blobs]
    )
    output_list = []
    for blobs in list_of_blobs:
        ...
        all_frames_padded = np.zeros(
            (blobs["all_frames"].shape[0], 3, f_max_shape[0], f_max_shape[1]),
            dtype=np.float32,
        )  # 7->6
