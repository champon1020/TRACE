...
def _get_image_blob(roidb):
    ...
    if (len(cfg.TRAIN.DATASETS) > 0 and cfg.TRAIN.DATASETS[0].find("vidvrd") >= 0) or (
        ...
        im_id_st = 0
    elif (len(cfg.TRAIN.DATASETS) > 0 and cfg.TRAIN.DATASETS[0].find("ag") >= 0) or (
        len(cfg.TEST.DATASETS) > 0 and cfg.TEST.DATASETS[0].find("ag") >= 0
    ):
        im_id_st = 1
    else:
        im_id_st = 1
    all_frames_blob, bf_cur_len, f_scale = get_frames_blob(
        ...
        scale_inds,
        im_id_st=im_id_st,
        half_frame_relative_path=cfg.HALF_FRAME_RELATIVE_PATH,
    )  ###!
...
def get_frames_blob(
    roidb, num_images, scale_inds, im_id_st=1, half_frame_relative_path=""
):
    all_frames_blob = []
    bf_cur_len = []
    f_scale = []
    if half_frame_relative_path == "sampled_frames":
        sufix_class = ".jpg"
    else:
        sufix_class = ".png"
    for i in range(num_images):
