...
parser.add_argument(
    ...
    default="rel_detection_range_12_15",
    type=str,
)
...
def vis_box(img, box, save_path, fname="obj_box"):
    ...
    for i in range(len(box)):
        ...
        srect = plt.Rectangle(
            (x, y), x1 - x, y1 - y, fill=False, edgecolor="b", linewidth=1
        )
        ax.add_patch(srect)
        # ax.text(x, y,
        #    color='white',
...
for ii, return_dict2 in enumerate(ret):
    ...
    if temporal_blob_conv_prd is not None:
        for i in range(len(temporal_blob_conv_prd)):
            visualize_feature_map(
                temporal_blob_conv_prd[i], save_dir, fname="temporal_feat_map" + str(i)
            )
    if roi is not None:
    ...
    if batch_A is not None:
        ...
        for i in range(len(fa)):
            if (son[i], fa[i]) in rid_f2s or (son[i], fa[i]) in rid_f2s:
                continue
        ...
        for i in range(len(batch_A)):
            ...
            visualize_feature_map(subim, os.path.join(save_dir, "subim"), str(i))
