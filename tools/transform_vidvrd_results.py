...
from datasets_rel.pytorch_misc import argsort_desc, intersect_2d
...
from vidvrd_utils.common import viou, voc_ap
...
def get_key_frame_set(
    full_video_path, prim_N_list, seg_step=15, seg_len=30, f_interval=3
):
    key_frame_set = dict()
    interval_name_list = []
    ...
    for i in tqdm(video_name_list):
        ...
        cur_interval_name_list = get_all_segment_interval_name(
            video_path, seg_step=seg_step, seg_len=seg_len
        )
    ...
    for j, st, cur_full_frame_path in tqdm(interval_name_list):
        ...
        for i in prim_N_list:
            middle_id = st + i
            sharp_id = sharpest_frame_calibration(
                middle_id, cur_full_frame_path, f_interval=f_interval
            )
...
def sharpest_frame_calibration(middle_id, full_frame_path, f_interval=3):
    sufixx = ".png"
    if (
        full_frame_path.find("sampled_frames") >= 0
        or full_frame_path.find("all_frames") >= 0
    ):
        sufixx = ".jpg"
    id_list = []
    val_list = []
...
def eval_detection_scores(gt_relations, pred_relations, viou_threshold):
    ...
    for pred_idx, pred_relation in enumerate(pred_relations):
        ...
        for gt_idx, gt_relation in enumerate(gt_relations):
            if not gt_detected[gt_idx] and tuple(pred_relation["triplet"]) == tuple(
                gt_relation["triplet"]
            ):
                s_iou = viou(
                    pred_relation["sub_traj"],
                    pred_relation["duration"],
                    gt_relation["sub_traj"],
                    gt_relation["duration"],
                )
                o_iou = viou(
                    pred_relation["obj_traj"],
                    pred_relation["duration"],
                    gt_relation["obj_traj"],
                    gt_relation["duration"],
                )
                ov = min(s_iou, o_iou)
...
def get_relation_insts(vid, tet):
    ...
    for anno_inst in anno["relation_instances"]:
        ...
        for dur in range(0, anno_inst["end_fid"] - anno_inst["begin_fid"], 15):
            if anno_inst["begin_fid"] + dur + 30 > anno_inst["end_fid"]:
                continue
            tem_seg_relation_insts = dict()
            tem_seg_relation_insts["triplet"] = inst["triplet"]
            tem_seg_relation_insts["duration"] = (
                anno_inst["begin_fid"] + dur,
                anno_inst["begin_fid"] + dur + 30,
            )
            ...
            if (
                anno_inst["begin_fid"] + dur,
                anno_inst["begin_fid"] + dur + 30,
            ) not in seg_relation_insts:
                seg_relation_insts[
                    (anno_inst["begin_fid"] + dur, anno_inst["begin_fid"] + dur + 30)
                ] = []
            seg_relation_insts[
                (anno_inst["begin_fid"] + dur, anno_inst["begin_fid"] + dur + 30)
            ].append(tem_seg_relation_insts)
...
def eval_rel_results(args, all_results, output_dir, topk=100, is_gt_traj=False):
    ...
    val_map_list_path = os.path.join(
        cfg.ROOT_DIR, "data", "vidvrd", "annotations/val_fname_list.json"
    )
    ...
    if args.sharp_frame:
        ...
        if os.path.exists(os.path.join("./data/vidvrd", "sharp_key_frame_set.pkl")):
            with open(
                os.path.join("./data/vidvrd", "sharp_key_frame_set.pkl"), "rb"
            ) as f:
                key_frame_set = pickle.load(f)
                f.close()
        else:
            key_frame_set = get_key_frame_set(
                full_video_path, N_list, seg_step=SEG_STEP, seg_len=30, f_interval=3
            )
            with open(
                os.path.join("./data/vidvrd", "sharp_key_frame_set.pkl"), "wb"
            ) as f:
    ...
    for prd_k in prd_k_set:
        ...
        for im_i, res in enumerate(tqdm(all_results)):
            ...
            ans_w = w
            if args.sharp_frame:
                ans_N_list, ans_w = judge_sharp_frame(
                    key_frame_set, res["image"], seg_step=SEG_STEP, seg_len=30
                )
            ...
            for iN, N in enumerate(ans_N_list):
                ...
                if (im_id_id - N) % SEG_STEP != 0 or im_id_id < N or im_id_id == 0:
                    continue
                ...
                if res["prd_scores"] is None:
                    ...
                    if sbj_to_pre_rois_overlaps.shape[1] >= 3:
                        s_top2_inds = np.argpartition(
                            sbj_to_pre_rois_overlaps[bad_inds], kth=2, axis=1
                        )
                        o_top2_inds = np.argpartition(
                            obj_to_pre_rois_overlaps[bad_inds], kth=2, axis=1
                        )
                ...
                if index not in mean_topk_dets[N]:
                    mean_topk_dets[N][index] = 0
                # print(N, index, mean_topk_dets[N][index])
                if (
                    mean_topk_dets[N][index] != 0
                ):  # assert False, 'Temporal Not Merged!'
        ...
        for N, v in mean_topk_dets.items():  # N=15, v={index1: ans1, index2: ans2}
            for index, single_video_res_list in tqdm(
                v.items()
            ):  # single_video_res_list=[(x1, x2, ...]
                if index not in mean_list_topk_dets:
                    ...
                    for quintuple in single_video_res_list:
                        ...
                        for i, new_quintuple in enumerate(mean_list_topk_dets[index]):
                            if (
                                quintuple[1] == new_quintuple[1]
                                and quintuple[2] == new_quintuple[2]
                            ):
        ...
        for k, v_list in topk_dets.items():
            for j, v in enumerate(v_list):
                ...
                for i, rel_info in enumerate(ans):
                    ...
                    topk_dets[k][j][1][i][0] = np.sum(
                        np.array(rel_info[0], dtype=np.float32)
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_dir", default="Outputs/ag_val_50_vidvrd", help="input_dir."
    )
    parser.add_argument(
        "--output_dir", default="Outputs/ag_val_50_vidvrd", help="output_dir."
    )
    parser.add_argument("--is_gt_traj", action="store_true", help="is_gt_traj.")
    parser.add_argument("--topk", default=200, type=int, help="output_dir.")
    parser.add_argument("--adapt_longtail_st", default=False, help="output_dir.")
    parser.add_argument(
        "--segment_groundtruth",
        default="data/vidvrd/annotations/test_seg_gt.json",
        help="A ground truth JSON file generated by yourself",
    )
    parser.add_argument("--is_train", action="store_true", help="is_test")
    parser.add_argument("--is_seg_test", action="store_true", help="is_test")
    parser.add_argument("--sharp_frame", action="store_true", help="sharp_frame")
    ...
    if os.path.exists(os.path.join(cfg.ROOT_DIR, args.input_dir, "rel_detections.pkl")):
        ...
        with open(
            os.path.join(cfg.ROOT_DIR, args.input_dir, "rel_detections.pkl"), "rb"
        ) as f:
            all_results = pickle.load(f)
            f.close()
    elif os.path.exists(
        os.path.join(cfg.ROOT_DIR, args.input_dir, "rel_detections_gt_boxes_sgcls.pkl")
    ):
        print("Loading rel_detections_gt_boxes_sgcls.pkl")
        with open(
            os.path.join(
                cfg.ROOT_DIR, args.input_dir, "rel_detections_gt_boxes_sgcls.pkl"
            ),
            "rb",
        ) as f:
            all_results = pickle.load(f)
    ...
    if not os.path.exists(
        os.path.join(args.output_dir, "transform_vidvrd_results.pkl")
    ):
        topk_dets = eval_rel_results(
            args,
            all_results,
            args.output_dir,
            # topk=args.topk, is_gt_traj=args.is_gt_traj)
            topk=2 * args.topk,
            is_gt_traj=args.is_gt_traj,
        )
    else:
        with open(
            os.path.join(args.output_dir, "transform_vidvrd_results.pkl"), "rb"
        ) as f:
    ...
    if args.is_seg_test:
        if os.path.exists(args.segment_groundtruth):
            print(
                "Loading segment ground truth from {}".format(args.segment_groundtruth)
            )
            ...
            with open(args.segment_groundtruth, "w") as fp:
                json.dump(gt_seg, fp)
                fp.close()
        serid_topk_dets = form_trans(
            obj_class_list, pred_class_list, topk_dets, is_gt_traj=args.is_gt_traj
        )
        print("Seg metric:")
        mean_ap, rec_at_n, mprec_at_n = evaluate(gt_seg, serid_topk_dets)
    else:
        if not os.path.exists(
            os.path.join(args.output_dir, "baseline_relation_prediction.json")
        ):
            video_relations = dict()
            for vid in tqdm(topk_dets.keys()):
                video_relations[vid] = greedy_relational_association(
                    obj_class_list,
                    pred_class_list,
                    topk_dets[vid],
                    max_traj_num_in_clip=args.topk,
                    is_gt=args.is_gt_traj,
                )
            with open(
                os.path.join(args.output_dir, "baseline_relation_prediction.json"), "w"
            ) as f:
            ...
            with open(
                os.path.join(args.output_dir, "baseline_relation_prediction.json"), "r"
            ) as f:
                output = json.load(f)
                f.close()
