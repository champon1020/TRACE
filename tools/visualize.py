...
def _compute_pred_matches(
    gt_triplets, pred_triplets, gt_boxes, pred_boxes, iou_thresh=0.5, phrdet=False
):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--st", help="Visualization Start", default=0, type=int)
    parser.add_argument("--num", help="Visualization Number", default=10, type=int)
    parser.add_argument("--no_do_vis", help="do not visualize", action="store_true")
    parser.add_argument(
        "--rel_class_recall", help="rel class recall.", action="store_true"
    )
    parser.add_argument("--phrdet", help="use phrdet.", action="store_true")
    parser.add_argument(
        "--dataset", help="Visualization Number", default="ag", type=str
    )
    parser.add_argument(
        "--filename", help="Visualization file", default="rel_detections_topk", type=str
    )
    parser.add_argument("--cnt_lim", help="Visualization Number", default=10, type=int)
    parser.add_argument("--lim", help="Visualization Number", default=0, type=int)
    ...
    with open(os.path.join("data", args.dataset, "annotations/objects.json"), "r") as f:
        ...
        f.close()
    with open(
        os.path.join("data", args.dataset, "annotations/predicates.json"), "r"
    ) as f:
    ...
    if args.dataset.find("ag") >= 0:
        val_map_list_path = os.path.join(
            "data", "ag", "annotations/test_videos_list.json"
        )
        ...
        val_map_list_path = os.path.join(
            "data", "vidvrd", "annotations/train_fname_list.json"
        )
        with open(val_map_list_path, "r") as f:
            val_map_list = json.load(f)
            f.close()
        ...
        val_map_list_path = os.path.join(
            "data", "vidvrd", "annotations/val_fname_list.json"
        )
        with open(val_map_list_path, "r") as f:
            val_map_list = json.load(f)
            f.close()
    ...
    for j, i in enumerate(val_map_list):
        ...
        for r in range(len(rel_list)):
            video_gt_cnt[r][file_real_name] = 0.0
            video_recalls[r][file_real_name] = {10: 0.0, 20: 0.0, 50: 0.0, 100: 0.0}
    ...
    for res_i in res[args.st : args.num]:
        ...
        det_labels_spo_top = np.vstack(
            (det_labels_s_top, det_labels_p_top, det_labels_o_top)
        ).transpose()

        if args.rel_class_recall:
            for i in gt_labels_prd:
                rel_class_gt_num[i] += 1
                video_gt_cnt[i][file_real_name] += 1.0
        ...
        for i in range(len(gt_boxes_sbj)):
            ...
            tri_info += [
                gt_labels_sbj[i].astype(np.int),
            ]
            tri_info += list(gt_boxes_obj[i, :].astype(np.int))
            tri_info += [
                gt_labels_obj[i].astype(np.int),
            ]
            tri_info += [
                gt_labels_prd[i].astype(np.int),
            ]
            gt_tri_info_set.add(tuple(tri_info))
            if (
                tuple(
                    list(gt_boxes_sbj[i, :].astype(np.int))
                    + [
                        gt_labels_sbj[i].astype(np.int),
                    ]
                )
            ...
            if (
                tuple(
                    list(gt_boxes_obj[i, :].astype(np.int))
                    + [
                        gt_labels_obj[i].astype(np.int),
                    ]
                )
            ...
            if len(pred_to_gt):
                ...
                if args.rel_class_recall:
                    for gt_i in match:
                        ...
                        video_recalls[gt_labels_prd[gt_i]][file_real_name][k] += 1.0
                ...
                for k in recalls:
                    ...
                    for gt_id in r_ans[k]:
                        ...
                        ax.text(
                            ...
                            color="white",
                            bbox=dict(
                                facecolor="orange", alpha=0.5, pad=0, edgecolor="none"
                            ),
                        )

                        tri_info = [x, y, x1, y1, gt_labels_sbj[gt_id].astype(np.int)]
                        if (
                            tuple([x, y, x1, y1, gt_labels_sbj[gt_id].astype(np.int)])
                            not in preserve_set_obj
                        ):
                            preserve_set_obj.add(
                                tuple(
                                    [x, y, x1, y1, gt_labels_sbj[gt_id].astype(np.int)]
                                )
                            )
                        ...
                        ax.text(
                            ...
                            color="white",
                            bbox=dict(
                                facecolor="blue", alpha=0.5, pad=0, edgecolor="none"
                            ),
                        )
                        p_name = (
                            rel_list[gt_labels_prd[gt_id].astype(np.int)]
                            + " "
                            + str(r_score[k][gt_id])
                        )
                        ...
                        if (
                            tuple([x, y, x1, y1, gt_labels_obj[gt_id].astype(np.int)])
                            not in preserve_set_obj
                        ):
                            preserve_set_obj.add(
                                tuple(
                                    [x, y, x1, y1, gt_labels_obj[gt_id].astype(np.int)]
                                )
                            )

                        tri_info += [
                            gt_labels_prd[gt_id].astype(np.int),
                        ]
                        preserve_set_rel.add(tuple(tri_info))

                        rel_l = lines.Line2D(
                            [s_cx, o_cx], [s_cy, o_cy], color="purple", linewidth=3
                        )
                        ax.add_line(rel_l)
                        ...
                        ax.text(
                            ...
                            color="white",
                            bbox=dict(
                                facecolor="purple", alpha=0.5, pad=0, edgecolor="none"
                            ),
                        )

                    dot = Digraph(
                        filename=(saved_name + "_" + str(k)).replace("/", "_")
                    )
                    ...
                    for gt_tri_info in gt_tri_info_set:
                        ...
                        st, ed = str(tuple([sx, sy, sx1, sy1, sn])), str(
                            tuple([ox, oy, ox1, oy1, on])
                        )
                    ...
                    dot.render(
                        os.path.join(
                            saved_path, (saved_name + "_" + str(k)).replace("/", "_")
                        ),
                        cleanup=True,
                    )

                    saved_file_name = (saved_name + "_" + str(k) + ".png").replace(
                        "/", "_"
                    )
                    plt.savefig(
                        os.path.join(saved_path, saved_file_name), bbox_inches="tight"
                    )
                    plt.close(fig)
    ...
    if args.rel_class_recall:
        for r in range(len(rel_list)):
            for file_real_name, v in video_recalls[r].items():
                for k, vals in v.items():
                    video_recalls[r][file_real_name][k] = float(vals) / (
                        float(video_gt_cnt[r][file_real_name]) + 1e-12
                    )
                    tot_recalls[r][k] += video_recalls[r][file_real_name][k]
            for k in tot_recalls[r]:
                tot_recalls[r][k] = float(tot_recalls[r][k]) / (
                    float(video_len) + 1e-12
                )
        ...
        for k in recalls:
            ...
            for i, gt_rel_num in enumerate(rel_class_gt_num):
                rel_class_recalls[i][k] = float(rel_class_recalls[i][k]) / (
                    float(gt_rel_num) + 1e-12
                )
