...
def batch_box_iou(boxes1, boxes2):
    ...
    return: [B, N, M]
    """
    area1 = (boxes1[:, :, 3] - boxes1[:, :, 1] + 1) * (
        boxes1[:, :, 2] - boxes1[:, :, 0] + 1
    )
    area2 = (boxes2[:, :, 3] - boxes2[:, :, 1] + 1) * (
        boxes2[:, :, 2] - boxes2[:, :, 0] + 1
    )
...
def cal_recall(predictions, k=100):
    ...
    return pred_sub_cls, pred_obj_cls, pred_rel_cls, pred_sub_boxes, pred_obj_boxes
...
def cal_hit(gt_labels, predictions, k=100):
    (
        ...
        pred_sub_boxes,
        pred_obj_boxes,
    ) = cal_recall(predictions, k=k)
    gt_sub_cls, gt_obj_cls, gt_rel_cls, gt_sub_boxes, gt_obj_boxes = cal_recall(
        gt_labels, k=None
    )
...
def get_predictions_in_current_frame(cur_frame_id, predictions, file_name):
    ...
    for triplet in predictions[file_name]:
        ...
        if cur_frame_id < st or cur_frame_id >= ed:
            continue
...
def plot_one_frame(instance_dict, rel_list, cur_frame_id, predictions, file_name, img):
    predictions_in_current_frame_list = get_predictions_in_current_frame(
        cur_frame_id, predictions, file_name
    )
...
def get_label(im_path, label_dict, file_name, predictions, selected_frames=None):
    ...
    for frame_id, boxes_list in enumerate(label_dict["trajectories"]):
        ...
        for xyxybox in boxes_list:
            ...
            cur_frame_instance_dict[tracklet_id] = dict(
                cls=xyxybox_cls, box=xyxybox_coord
            )
        cur_rel_list = get_gt_rel_in_current_frame(
            label_dict["relation_instances"], frame_id
        )
        if selected_frames is None or frame_id in selected_frames:
            img = mpimg.imread(im_path + "/" + "{:06d}".format(frame_id) + ".png")
            plot_one_frame(
                cur_frame_instance_dict,
                cur_rel_list,
                frame_id,
                predictions,
                file_name,
