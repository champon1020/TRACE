...
def dump_frames(args, frame_list_file="frame_list.txt", st_id=1):
    ...
    for v in tqdm(video2frames):
        ...
        if args.sampled_frames:
            ...
            tmp_ans = get_sampled_frames(
                f_id_list, args.half_len, frames_store_type, st_id
            )
            # print(float(len(tmp_ans)) / (float(len(f_id_list))+1e-12))
            if args.rid_of_half_len > 0:
                rid_tmp_ans = get_sampled_frames(
                    f_id_list, args.rid_of_half_len, frames_store_type, st_id
                )
        ...
        if not os.path.exists(curr_frame_dir):
            ...
            if frames_store_type == "jpg" and args.high_quality:
                if ignore_editlist:
                    os.system(
                        "ffmpeg -ignore_editlist 1 -loglevel panic -i %s/%s -start_number %d -qscale:v 2 %s/%%06d.%s"
                        % (video_dir, v, st_id, curr_frame_dir, frames_store_type)
                    )
                else:
                    os.system(
                        "ffmpeg -loglevel panic -i %s/%s -start_number %d -qscale:v 2 %s/%%06d.%s"
                        % (video_dir, v, st_id, curr_frame_dir, frames_store_type)
                    )
                ...
                if ignore_editlist:
                    os.system(
                        "ffmpeg -ignore_editlist 1 -loglevel panic -i %s/%s -start_number %d %s/%%06d.%s"
                        % (video_dir, v, st_id, curr_frame_dir, frames_store_type)
                    )
                else:
                    os.system(
                        "ffmpeg -loglevel panic -i %s/%s -start_number %d %s/%%06d.%s"
                        % (video_dir, v, st_id, curr_frame_dir, frames_store_type)
                    )
            ...
            warnings.warn(
                "Frame directory %s already exists. Skipping dumping into this directory."
                % curr_frame_dir,
                RuntimeWarning,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--video_dir",
        default="data/ag/videos",
        help="Folder containing Charades videos.",
    )
    parser.add_argument(
        "--frame_dir",
        default="data/ag/frames",
        help="Root folder containing frames to be dumped.",
    )
    parser.add_argument(
        "--annotation_dir",
        default="data/ag/annotations",
        help=(
            "Folder containing annotation files, including object_bbox_and_relationship.pkl, "
    ...
    parser.add_argument(
        "--frames_store_type", default="png", help="The type for storing frames."
    )
    parser.add_argument(
        "--ignore_editlist",
        action="store_true",
    ...
    parser.add_argument(
        "--high_quality",
        help="high_quality, 6 times bigger (jpg_low: 3kb, jpg_high: 20kb, png: 170kb).",
        action="store_true",
    )
    parser.add_argument("--half_len", help="half_len", default=13, type=int)
    # default=8, type=int)
    parser.add_argument(
        "--sampled_frames", action="store_true", help="dump sampled_frames"
    )
    parser.add_argument(
        "--frame_list_file", default="frame_list.txt", help="frame_list_file."
    )
    parser.add_argument("--st_id", default=1, help="st_id.")
    parser.add_argument(
        "--rid_of_half_len", help="rid_of_half_len", default=-1, type=int
    )
