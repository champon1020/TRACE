...
def get_obj_det(obj_anno_list):
    ...
    for i in range(len(obj_det)):
        ...
        obj_det[i]["category_id"] = np.array(
            obj_det[i]["category_id"], dtype=np.float32
        )
    return obj_det
...
def get_gtlabel(obj_boxes, obj_gt_boxes, obj_gt_labels):
    ...
    obj_labels[fg_inds] = (
        np.array(obj_gt_labels[obj_argmaxes[fg_inds]], dtype=np.int32) + 1
    )
    obj_labels[bg_inds] = 0
    return obj_labels


def save_pretrain_rois(
    train_frame_mapping_dict,
    val_frame_mapping_dict,
    train_obj_det,
    test_obj_det,
    out_rpath="pre_processed_boxes",
    out_dir="data/vidvrd",
    anno_dir="data/vidvrd",
    rpath="traj_cls",
):
    ...
    for file_name in tqdm(video_anno_list):
        ...
        for frame_anno in frame_anno_list:
            all_st, all_ed = int(frame_anno.split("-")[-3]), int(
                frame_anno.split("-")[-2]
            )
            if frame_anno.split(".")[-1] == "json":
                ...
                to_save = [
                    {"rois": [], "labels_int32": [], "score": []}
                    for _ in range(all_st, all_ed)
                ]
                ...
                for anno in anno_list:
                    ...
                    for frame_id in range(anno["pstart"], anno["pend"]):
                        ...
                        x1, y1, x2, y2 = box
                        np_box = np.array([0.0, x1, y1, x2, y2], dtype=np.float32)
                ...
                for frame_id in _mid:
                    frame_name = (
                        cur_seg_name + ".mp4" + "/" + "{:06d}".format(frame_id) + ".png"
                    )
                    ...
                    val_dict["score"] = np.hstack(
                        (
                            np.zeros(len(val_dict["score"])).reshape(-1, 1),
                            val_dict["score"],
                        )
                    )
                    # val_dict['prd_category'] = np.array(val_dict['prd_category'], dtype=np.int32) + 1
                    objlabels = get_gtlabel(
                        val_dict["rois"][:, 1:],
                    ...
                    if not os.path.exists(
                        os.path.join(out_dir, out_rpath, out_split, saved_id)
                    ):
                        with open(
                            os.path.join(out_dir, out_rpath, out_split, saved_id), "wb"
                        ) as f:  # save
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_dir", default="detection_models/vidvrd", help="out_dir.")
    parser.add_argument("--out_rpath", default="pre_processed_boxes", help="out_rpath.")
    parser.add_argument("--rpath", default="traj_cls", help="rpath.")
    ...
    with open(os.path.join("data/vidvrd/annotations", "detections_val.json"), "r") as f:
        ...
        f.close()
    with open(
        os.path.join("data/vidvrd/annotations", "detections_train.json"), "r"
    ) as f:
        train_obj_anno_list = json.load(f)
        f.close()
    with open(
        os.path.join("data/vidvrd/annotations", "val_fname_mapping.json"), "r"
    ) as f:
        val_frame_mapping_dict = json.load(f)
        f.close()
    with open(
        os.path.join("data/vidvrd/annotations", "train_fname_mapping.json"), "r"
    ) as f:
