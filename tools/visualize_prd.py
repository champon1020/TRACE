...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(args.output_dir, "vis_prd")):
        os.mkdir(os.path.join(args.output_dir, "vis_prd"))
    saved_path = os.path.join(args.output_dir, "vis_prd")
    ...
    rel_class_recalls = [{20: 0, 50: 0, 100: 0} for i in range(len(rel_list))]
    ...
    for res_i in res[args.st : args.num]:
        r_ans = {20: 0, 50: 0, 100: 0}
        r_score = {20: 0, 50: 0, 100: 0}
        f_name = res_i["image"]
        ...
        if not args.no_do_vis:
            ...
            for k in recalls:
                ...
                for i in range(k):
                    if i >= len(det_boxes_s_top):
                        continue
                    x, y, x1, y1 = det_boxes_s_top[i].astype(np.int)

                    s_name = obj_list[det_labels_s_top[i]]

                    s_cx, s_cy = (x + x1) // 2, (y + y1) // 2

                    srect = plt.Rectangle(
                        (x, y), x1 - x, y1 - y, fill=False, edgecolor="b", linewidth=3
                    )
                    ax.add_patch(srect)
                    ...
                    x, y, x1, y1 = det_boxes_o_top[i].astype(np.int)
                    o_name = obj_list[det_labels_o_top[i]]
                    ...
                    p_name = (
                        rel_list[det_labels_p_top[i].astype(np.int)]
                        + " "
                        + str(det_scores_top[i])
                    )
