...
from core.config import (assert_and_infer_cfg, cfg, merge_cfg_from_file,
                         merge_cfg_from_list)
...
def parse_args():
    ...
    parser.add_argument(
        "--do_vis",
        dest="do_vis",
        help="visualize the last layer of conv_body",
        action="store_true",
    )
    parser.add_argument(
        "--do_special",
    ...
    parser.add_argument(
        "--use_gt_boxes",
        dest="use_gt_boxes",
        help="use gt boxes for sgcls/prdcls",
        action="store_true",
    )
    parser.add_argument(
        "--use_gt_labels",
    ...
    parser.add_argument(
        "--eva_map", dest="eva_map", help="eva_map", action="store_true"
    )
...
if __name__ == "__main__":
    ...
    if args.dataset == "oi_rel":
        ...
        cfg.MODEL.NUM_CLASSES = 101
        cfg.MODEL.NUM_PRD_CLASSES = 70  # exclude background
    elif args.dataset == "vidvrd_train":
        cfg.TEST.DATASETS = ("vidvrd_train",)
        cfg.MODEL.OBJ_CLASS_FILE_NAME = os.path.join(
    ...
    if not cfg.MODEL.RUN_BASELINE:
        assert bool(args.load_ckpt) ^ bool(
            args.load_detectron
    ...
    if args.use_gt_boxes:
        if args.use_gt_labels:
            det_file = os.path.join(
                args.output_dir, "rel_detections_gt_boxes_prdcls.pkl"
            )
        else:
            det_file = os.path.join(
                args.output_dir, "rel_detections_gt_boxes_sgcls.pkl"
            )
    else:
    ...
    if os.path.exists(det_file):
        ...
        if (
            args.dataset.find("vg") >= 0
            or args.dataset.find("vrd") >= 0
            or args.dataset.find("ag") >= 0
        ) and not args.eva_map:
            task_evaluation_vg_and_vrd.eval_rel_results(
                all_results, args.output_dir, args.topk, args.do_val
            )
        else:
            task_evaluation_sg.eval_rel_results(
                all_results,
                args.output_dir,
                args.topk,
                args.do_val,
