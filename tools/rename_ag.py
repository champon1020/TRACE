...
def process_vrd_split(in_split, out_split, split_set, obj_dict, pred_dict, issave=True):
    ...
    def get_triplet(v, person_bbox_anns, rel_class, obj_dict, pred_dict):
        ...
        h = v["bbox"][3]
        triplet_info["object"] = {
            "category": obj_class2int(v["class"], obj_dict),
            "bbox": [int(y), int(y + h), int(x), int(x + w)],
        }
        ...
        triplet_info["subject"] = {
            "category": 0,
            "bbox": [int(y), int(y1), int(x), int(x1)],
        }
        return triplet_info
    ...
    for k, v in tqdm(relation_anns.items()):
        ...
        v_list = []
        for obj_info in v:
            if obj_info["metadata"]["set"] != in_split:
                print(obj_info["metadata"]["set"], in_split)

            if obj_info["attention_relationship"] is not None:
                for rel_class in obj_info["attention_relationship"]:
                    tri = get_triplet(
                        obj_info, person_bbox_anns[k], rel_class, obj_dict, pred_dict
                    )
                    v_list.append(tri)
            if obj_info["spatial_relationship"] is not None:
    ...
    if issave:
        ...
        with open(
            "data/ag/annotations/new_annotations_" + out_split + ".json", "w"
        ) as outfile:
            json.dump(new_anns, outfile)

    convert_anno(out_split, vrd_anns, obj_dict, fname_list)
...
def convert_anno(split, vrd_anns, obj_dict, fname_list):
    ...
    for f, anns in tqdm(vrd_anns.items()):
        ...
        im_h, im_w, C = cv2.imread(f_path, cv2.IMREAD_COLOR).shape
...
if __name__ == "__main__":
    ...
    issave = True
    process_vrd_split("test", "val", test_split, obj_dict, pred_dict, issave)
    process_vrd_split("train", "train", train_split, obj_dict, pred_dict, issave)
