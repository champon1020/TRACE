...
def get_box_from_track(tracklet_clip, obj_list, tid, obj_class_list, video_id):
    ...
    ans = {
        "category": obj_class2int(category, obj_class_list),
        "bbox": [y1, y2, x1, x2],
    }
    return ans
...
def process_vrd_split(pred_class_list, obj_class_list, out_split="train"):
            ...
            for t in range(st, ed):
                ...
                for tracklet_clip in cur_frame_tracks_list:
                    if tracklet_clip["tid"] == s_tid:
                        triplet_info["subject"] = get_box_from_track(
                            tracklet_clip, obj_list, s_tid, obj_class_list, video_id
                        )
                    if tracklet_clip["tid"] == o_tid:
                        triplet_info["object"] = get_box_from_track(
                            tracklet_clip, obj_list, o_tid, obj_class_list, video_id
                        )
                if t in sgg:
    ...
    with open(
        "data/vidvrd/annotations/new_annotations_" + out_split + ".json", "w"
    ) as outfile:
