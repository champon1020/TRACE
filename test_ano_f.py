...
for i, v in an_list.items():
    ...
    if len(g) not in pair_count:
        pair_count[len(g)] = 0.0
    pair_count[len(g)] += 1.0
...
print(
    "{} val relations density. ".format(float(all_cnt / len(an_list)))
)  # 8.451416826238955 val relations density.
print(max_tri)  # 31
print(max_pair)  # 9
print(max_r_pair)  # 9
...
print(
    "number of all count pairs: {}, Expect recall: {}".format(
        n_allcountpair / tot * 100,
        n_allcountpair / tot * 100 + (1 - n_allcountpair / tot) * 50,
    )
)
print()
...
r = float(cnt) / (1e-12 + float(all_cnt)) * 100
print(
    "{} train relations iou>0. ".format(r)
)  # 87.28015177142264 train relations iou>0.
print(
    "{} train relations density. ".format(float(all_cnt / len(an_list)))
)  # 6.96152935205549 train relations density.
print(max_tri)  # 29
print(max_pair)  # 8
print(max_r_pair)  # 8
print(mean_r_pair / valid_g_cnt)  # 3.28
...
print(
    "Expect hit: {}".format(np.array(sum_prob, dtype=np.float32).sum() / float(all_cnt))
)
