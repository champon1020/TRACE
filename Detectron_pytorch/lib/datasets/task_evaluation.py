...
def evaluate_keypoints(dataset, all_boxes, all_keyps, output_dir):
    ...
    logger.info("Evaluating detections")
    not_comp = not cfg.TEST.COMPETITION_MODE
    assert dataset.name.startswith(
        "keypoints_coco_"
    ), "Only COCO keypoints are currently supported"
    coco_eval = json_dataset_evaluator.evaluate_keypoints(
        dataset, all_boxes, all_keyps, output_dir, use_salt=not_comp, cleanup=not_comp
    )
...
def check_expected_results(results, atol=0.005, rtol=0.1):
    ...
    for dataset, task, metric, expected_val in cfg.EXPECTED_RESULTS:
        ...
        assert metric in results[dataset][task], "Metric {} not in results".format(
            metric
        )
        ...
        if err > tol:
            ...
            if cfg.EXPECTED_RESULTS_EMAIL != "":
                ...
                job_name = (
                    os.environ["DETECTRON_JOB_NAME"]
                    if "DETECTRON_JOB_NAME" in os.environ
                    else "<unknown>"
                )
                job_id = (
                    os.environ["WORKFLOW_RUN_ID"]
                    if "WORKFLOW_RUN_ID" in os.environ
                    else "<unknown>"
                )
                body = [
                    "Name:",
