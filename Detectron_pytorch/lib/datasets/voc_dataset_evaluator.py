...
from datasets.dataset_catalog import DATASETS, DEVKIT_DIR
...
def evaluate_boxes(
    json_dataset, all_boxes, output_dir, use_salt=True, cleanup=True, use_matlab=False
):
...
def _write_voc_results_files(json_dataset, all_boxes, salt):
    ...
    assert os.path.exists(image_set_path), "Image set path does not exist: {}".format(
        image_set_path
    )
...
def _do_matlab_eval(json_dataset, salt, output_dir="output"):
    ...
    cmd += "voc_eval('{:s}','{:s}','{:s}','{:s}'); quit;\"".format(
        info["devkit_path"], "comp4" + salt, info["image_set"], output_dir
    )
    logger.info("Running:\n{}".format(cmd))
...
def voc_info(json_dataset):
    ...
    assert os.path.exists(devkit_path), "Devkit directory {} not found".format(
        devkit_path
    )
