...
def ResNet50_conv5_body_dilation2():
    return ResNet_convX_body((3, 4, 6, 3), dilation_num=2)
...
def ResNet101_conv5_body_dilation2():
    return ResNet_convX_body((3, 4, 23, 3), dilation_num=2)
...
def ResNet152_conv5_body_dilation2():
    return ResNet_convX_body((3, 8, 36, 3), dilation_num=2)
...
class ResNet_convX_body(nn.Module):
    def __init__(self, block_counts, dilation_num=None):
        super().__init__()
        ...
        self.res2, dim_in = add_stage(
            dim_in, 256, dim_bottleneck, block_counts[0], dilation=1, stride_init=1
        )
        self.res3, dim_in = add_stage(
            dim_in, 512, dim_bottleneck * 2, block_counts[1], dilation=1, stride_init=2
        )
        self.res4, dim_in = add_stage(
            dim_in, 1024, dim_bottleneck * 4, block_counts[2], dilation=1, stride_init=2
        )
        if len(block_counts) == 4:
            if dilation_num is None:
                dilation_num = cfg.RESNETS.RES5_DILATION
            stride_init = 2 if dilation_num == 1 else 1
            ...
            self.spatial_scale = 1 / 32 * dilation_num
        else:
    ...
    def _init_modules(self):
        ...
        self.apply(
            lambda m: freeze_params(m) if isinstance(m, mynn.AffineChannel2d) else None
        )
...
class ResNet_roi_conv5_head(nn.Module):
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        ...
        self.res5, self.dim_out = add_stage(
            dim_in, 2048, dim_bottleneck * 8, 3, dilation=1, stride_init=stride_init
        )
    ...
    def detectron_weight_mapping(self):
        mapping_to_detectron, orphan_in_detectron = residual_stage_detectron_mapping(
            self.res5, "res5", 3, 5
        )
        return mapping_to_detectron, orphan_in_detectron
...
def add_stage(inplanes, outplanes, innerplanes, nblocks, dilation=1, stride_init=2):
    ...
    for _ in range(nblocks):
        res_blocks.append(
            add_residual_block(inplanes, outplanes, innerplanes, dilation, stride)
        )
        inplanes = outplanes
        stride = 1
...
def basic_gn_shortcut(inplanes, outplanes, stride):
    return nn.Sequential(
        nn.Conv2d(inplanes, outplanes, kernel_size=1, stride=stride, bias=False),
        nn.GroupNorm(
            net_utils.get_group_gn(outplanes), outplanes, eps=cfg.GROUP_NORM.EPSILON
        ),
    )
        ...
        OrderedDict(
            [
                ...
                (
                    "gn1",
                    nn.GroupNorm(
                        net_utils.get_group_gn(64), 64, eps=cfg.GROUP_NORM.EPSILON
                    ),
                ),
                ("relu", nn.ReLU(inplace=True)),
                ("maxpool", nn.MaxPool2d(kernel_size=3, stride=2, padding=1)),
...
class bottleneck_gn_transformation(nn.Module):
    ...
    def __init__(
        ...
        self.gn1 = nn.GroupNorm(
            net_utils.get_group_gn(innerplanes), innerplanes, eps=cfg.GROUP_NORM.EPSILON
        )
...
def residual_stage_detectron_mapping(module_ref, module_name, num_blocks, res_id):
    ...
    for blk_id in range(num_blocks):
        ...
        if getattr(module_ref[blk_id], "downsample"):
            dtt_bp = (
                detectron_prefix + "_branch1"
            )  # short for "detectron_branch_prefix"
        ...
        for i, c in zip([1, 2, 3], ["a", "b", "c"]):
            ...
            orphan_in_detectron.append(dtt_bp + "_b")
            mapping_to_detectron[
                my_prefix + "." + norm_suffix[1:] + "%d.weight" % i
            ] = (dtt_bp + norm_suffix + "_s")
            mapping_to_detectron[my_prefix + "." + norm_suffix[1:] + "%d.bias" % i] = (
                dtt_bp + norm_suffix + "_b"
            )
