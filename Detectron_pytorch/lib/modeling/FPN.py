...
from modeling.collect_and_distribute_fpn_rpn_proposals import \
...
def fpn_ResNet50_conv5_body():
    return fpn(ResNet.ResNet50_conv5_body, fpn_level_info_ResNet50_conv5())


def fpn_ResNet50_conv5_P2only_body():
    return fpn(ResNet.ResNet50_conv5_body, fpn_level_info_ResNet50_conv5(), P2only=True)


def fpn_ResNet101_conv5_body():
    return fpn(ResNet.ResNet101_conv5_body, fpn_level_info_ResNet101_conv5())


def fpn_ResNet101_conv5_P2only_body():
    return fpn(
        ResNet.ResNet101_conv5_body, fpn_level_info_ResNet101_conv5(), P2only=True
    )


def fpn_ResNet152_conv5_body():
    return fpn(ResNet.ResNet152_conv5_body, fpn_level_info_ResNet152_conv5())


def fpn_ResNet152_conv5_P2only_body():
    return fpn(
        ResNet.ResNet152_conv5_body, fpn_level_info_ResNet152_conv5(), P2only=True
    )
...
class fpn(nn.Module):
    ...
    def __init__(self, conv_body_func, fpn_level_info, P2only=False):
        ...
        self.num_backbone_stages = len(fpn_level_info.blobs) - (
        ...
        if cfg.FPN.USE_GN:
            self.conv_top = nn.Sequential(
                ...
                nn.GroupNorm(
                    net_utils.get_group_gn(fpn_dim), fpn_dim, eps=cfg.GROUP_NORM.EPSILON
                ),
            )
        else:
            self.conv_top = nn.Conv2d(fpn_dim_lateral[0], fpn_dim, 1, 1, 0)
            ...
            if cfg.FPN.USE_GN:
                self.posthoc_modules.append(
                    nn.Sequential(
                        ...
                        nn.GroupNorm(
                            net_utils.get_group_gn(fpn_dim),
                            fpn_dim,
    ...
    def _init_weights(self):
        ...
        for child_m in self.children():
            if not isinstance(child_m, nn.ModuleList) or not isinstance(
                child_m[0], topdown_lateral_module
            ):
    ...
    def detectron_weight_mapping(self):
        (
            conv_body_mapping,
            orphan_in_detectron,
        ) = self.conv_body.detectron_weight_mapping()
    ...
    def forward(self, x):
        ...
        if hasattr(self, "extra_pyramid_modules"):
            ...
            for module in self.extra_pyramid_modules[1:]:
                fpn_output_blobs.insert(
                    0, module(F.relu(fpn_output_blobs[0], inplace=True))
                )
    ...
    def __init__(self, dim_in_top, dim_in_lateral):
        ...
        if cfg.FPN.USE_GN:
            self.conv_lateral = nn.Sequential(
                ...
                nn.GroupNorm(
                    net_utils.get_group_gn(self.dim_out),
                    self.dim_out,
                    eps=cfg.GROUP_NORM.EPSILON,
                ),
            )
    ...
    def __init__(self, dim_in, spatial_scales):
        ...
        for lvl in range(k_min, k_max + 1):
            ...
            lvl_anchors = generate_anchors(
                stride=2.0**lvl,
                sizes=(cfg.FPN.RPN_ANCHOR_START_SIZE * 2.0 ** (lvl - k_min),),
    ...
    def forward(self, blobs_in, im_info, roidb=None):
        ...
        for lvl in range(k_min, k_max + 1):
            ...
            if not self.training or cfg.MODEL.FASTER_RCNN:
                ...
                fpn_rpn_rois, fpn_rpn_roi_probs = self.GenerateProposals_modules[
                    lvl - k_min
...
def fpn_rpn_losses(**kwargs):
    ...
    for lvl in range(cfg.FPN.RPN_MIN_LEVEL, cfg.FPN.RPN_MAX_LEVEL + 1):
        ...
        rpn_bbox_inside_weights_fpn = kwargs["rpn_bbox_inside_weights_wide_fpn" + slvl][
            :, :, :h, :w
        ]
        rpn_bbox_outside_weights_fpn = kwargs[
            "rpn_bbox_outside_weights_wide_fpn" + slvl
        ][:, :, :h, :w]

        if cfg.RPN.CLS_ACTIVATION == "softmax":
            rpn_cls_logits_fpn = (
                kwargs["rpn_cls_logits_fpn" + slvl]
                .view(b, 2, c // 2, h, w)
                .permute(0, 2, 3, 4, 1)
                .contiguous()
                .view(-1, 2)
            ...
            loss_rpn_cls_fpn = F.binary_cross_entropy_with_logits(
                kwargs["rpn_cls_logits_fpn" + slvl],
                rpn_labels_int32_fpn.float(),
                weight,
                reduction="sum",
            )
            ###!!!size_average=False)
            loss_rpn_cls_fpn /= (
                cfg.TRAIN.RPN_BATCH_SIZE_PER_IM * cfg.TRAIN.IMS_PER_BATCH
            )
        ...
        loss_rpn_bbox_fpn = net_utils.smooth_l1_loss(
            kwargs["rpn_bbox_pred_fpn" + slvl],
            rpn_bbox_targets_fpn,
            rpn_bbox_inside_weights_fpn,
            rpn_bbox_outside_weights_fpn,
            beta=1 / 9,
        )
...
FpnLevelInfo = collections.namedtuple(
    "FpnLevelInfo", ["blobs", "dims", "spatial_scales"]
)
...
def fpn_level_info_ResNet50_conv5():
    return FpnLevelInfo(
        ...
        spatial_scales=(1.0 / 32.0, 1.0 / 16.0, 1.0 / 8.0, 1.0 / 4.0),
    )
