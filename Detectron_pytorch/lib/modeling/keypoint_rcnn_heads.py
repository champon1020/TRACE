    ...
    def detectron_weight_mapping(self):
        ...
        if cfg.KRCNN.USE_DECONV:
            detectron_weight_mapping.update(
                {"deconv.weight": "kps_deconv_w", "deconv.bias": "kps_deconv_b"}
            )
        ...
        if self.upsample_heatmap:
            ...
            blob_name = "kps_score"
        detectron_weight_mapping.update(
            {"classify.weight": blob_name + "_w", "classify.bias": blob_name + "_b"}
        )
...
def keypoint_losses(
    kps_pred, keypoint_locations_int32, keypoint_weights, keypoint_loss_normalizer=None
):
    """Mask R-CNN keypoint specific losses."""
    device_id = kps_pred.get_device()
    kps_target = Variable(
        torch.from_numpy(keypoint_locations_int32.astype("int64"))
    ).cuda(device_id)
...
class roi_pose_head_v1convX(nn.Module):
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Conv2d):
            if cfg.KRCNN.CONV_INIT == "GaussianFill":
                ...
                ValueError(
                    "Unexpected cfg.KRCNN.CONV_INIT: {}".format(cfg.KRCNN.CONV_INIT)
                )
    ...
    def detectron_weight_mapping(self):
        ...
        for i in range(cfg.KRCNN.NUM_STACKED_CONVS):
            detectron_weight_mapping[
                "conv_fcn.%d.weight" % (2 * i)
            ] = "conv_fcn%d_w" % (i + 1)
            detectron_weight_mapping["conv_fcn.%d.bias" % (2 * i)] = "conv_fcn%d_b" % (
                i + 1
            )
