...
class voc2coco:
    ...
    def _load_annotation(self, ids=[]):
        ...
        for index in ids:
            ...
            json_file = os.path.join(
                self.data_path, "Segmentation_json", filename + ".json"
            )
            ...
            if os.path.exists(json_file):
                ...
                assert type(seg_data) == type(
                    dict()
                ), "annotation file format {} not supported".format(type(seg_data))
            ...
            # LabelImg label file .xml
                ...
                for obj in objs:
                    ...
                    one_ann_msg = {
                        ...
                        "category_id": self.categories_to_ids_map[
                            obj.find("name").text
                        ],
                        "id": annotation_id,
                        "ignore": 0,
    ...
    def _save_json_file(self, filename=None, data=None):
        ...
        if not filename.endswith(".json"):
            filename += ".json"
        assert type(data) == type(dict()), "data format {} not supported".format(
            type(data)
        )
    ...
    def _get_indexs_by_image_set(self, image_set=None):
        ...
        if image_set is None:
            ...
            image_set_path = os.path.join(
                self.image_set_path, "Main", image_set + ".txt"
            )
            assert os.path.exists(image_set_path), "Path does not exist: {}".format(
                image_set_path
            )
