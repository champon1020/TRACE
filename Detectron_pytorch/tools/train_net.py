...
from roi_data.loader import MinibatchSampler, RoiDataLoader, collate_minibatch
...
def main():
    ...
    print(
        "Batch size change from {} (in config file) to {}".format(
            original_batch_size, args.batch_size
        )
    )
    print(
        "NUM_GPUs: %d, TRAIN.IMS_PER_BATCH: %d"
    ...
    print(
        "Adjust BASE_LR linearly according to batch size change: {} --> {}".format(
            old_base_lr, cfg.SOLVER.BASE_LR
        )
    )
    ...
    if args.load_ckpt:
        ...
        if args.resume:
            assert (
                checkpoint["iters_per_epoch"] == train_size // args.batch_size
    ...
    try:
        ...
        for args.epoch in range(args.start_epoch, args.start_epoch + args.num_epochs):
            ...
            if (
                ...
                and args.epoch == args.lr_decay_epochs[0]
                and args.start_iter == 0
            ):
